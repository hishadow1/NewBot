...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password,
                                   disk_gb=30, hostname=None, progress_fn=None):
    """
    Create a VPS container from the darknodes-vps image.

    progress_fn: optional async callable(step_name: str) called at each major
    deployment stage so the caller can edit a live Discord progress embed.
    ...
    - /var/run/docker.sock is mounted so Docker CLI works via the host daemon.
    ...
    await _progress("Building Docker image")
    ...
    #    - /var/run/docker.sock gives the container Docker CLI access via the host daemon
    ...
    base_flags = (
        ...
        f"--privileged "
        f"--cgroupns=host "
        f"--tmpfs /run:exec,mode=755,size=128m "
        f"--tmpfs /run/lock:size=64m "
        f"--tmpfs /tmp:exec,size=256m "
    ...
    await _progress("Creating container")
    try:
    ...
    await _progress("Booting systemd")
    await asyncio.sleep(3)

    # 5. Configure the VPS — password, hostname, docker socket permissions, SSH
    await _progress("Configuring VPS")
    try:
    ...
    # 6. Full verification — on failure keep the container for diagnosis
    await _progress("Verifying system")
...
async def configure_vps(container_name: str, password: str, hostname: str = None):
    ...
    Steps:
      ...
      4. Fix Docker socket permissions so the CLI works immediately.
...
# ── 1.5. Mask rescue/emergency units ─────────────────────────────────────────
# These units always fail inside Docker and cause is-system-running to report
# "maintenance" instead of "degraded". Masking them is idempotent and safe.
...
# The host socket is mounted at /var/run/docker.sock.
# Make it world-readable/writable so the docker CLI works without sudo.
...
"""
    ...
    stdout, stderr, rc = await docker_exec(container_name, configure_script, timeout=90)
...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    """
    Verify a newly-created container is fully functional.

    Returns (all_passed: bool, human_readable_report: str).
    Does NOT clean up on failure — the caller handles that (and keeps the container).

    Critical checks (deployment fails if any one fails):
      ✓ PID 1 is systemd
      ✓ systemctl is responsive
      ✓ Hostname matches expected value

    Optional / warning checks (logged but do not fail deployment):
      ✓ SSH daemon running    (internal use — not exposed to users)
      ✓ docker version        (CLI talks to host daemon)
      ✓ docker ps             (host daemon accessible)
      ✓ docker images         (host daemon accessible)
      ✓ tmate installed       (access method)
      ✓ sshx installed        (access method)
      ✓ docker run hello-world (network + daemon sanity)
    """
    ...
    # Allow systemd services a moment to fully activate
    await asyncio.sleep(4)

    # ── Critical checks ───────────────────────────────────────────────────────
    pid1_ok      = await run_check("PID 1 is systemd", "cat /proc/1/comm",         "systemd")
    systemctl_ok = await run_check("systemctl",         "systemctl --version 2>&1", "systemd")
    hostname_ok  = await run_check("Hostname",          "hostname",                 hostname or VPS_HOSTNAME)

    # ── SSH daemon (optional — service runs internally but is not exposed to users) ──
    ssh_ok = await run_check("SSH daemon", "pgrep -x sshd && echo sshd_running", "sshd_running")
    if not ssh_ok:
        await asyncio.sleep(5)
        ssh_ok = await run_check("SSH daemon (retry)", "pgrep -x sshd && echo sshd_running", "sshd_running")

    # ── Docker checks (optional — socket permission may still be propagating) ──
    docker_ver_ok = await run_check("docker version", "docker version 2>&1",                     "Server")
    docker_ps_ok  = await run_check("docker ps",      "docker ps --format '{{.ID}}' 2>&1",       None)
    docker_img_ok = await run_check("docker images",  "docker images --format '{{.Repository}}' 2>&1", None)

    if not (docker_ver_ok and docker_ps_ok and docker_img_ok):
        await asyncio.sleep(3)
        if not docker_ver_ok:
            docker_ver_ok = await run_check("docker version (retry)", "docker version 2>&1", "Server")
        if not docker_ps_ok:
            docker_ps_ok  = await run_check("docker ps (retry)", "docker ps --format '{{.ID}}' 2>&1", None)
        if not docker_img_ok:
            docker_img_ok = await run_check("docker images (retry)", "docker images --format '{{.Repository}}' 2>&1", None)

    # ── Access-method checks (optional) ──────────────────────────────────────
    await run_check("tmate installed", "command -v tmate && tmate -V 2>&1 | head -1", None, timeout=10)
    await run_check("sshx installed",  "command -v sshx 2>&1",                        None, timeout=10)

    # docker run hello-world (optional — may be slow on first pull)
    await run_check(
        "docker run hello-world",
        "docker run --rm hello-world 2>&1 | grep -i 'hello from docker' || true",
        None,
        timeout=60
    )

    # ── Build human-readable report ───────────────────────────────────────────
    # Only these three failing will block deployment.
    _CRITICAL = frozenset({"PID 1 is systemd", "systemctl", "Hostname"})
    ...
    if all_passed:
        ...
        logger.error(f"VPS {container_name} verification FAILED — critical: {critical_failed}.\n{report}")
...
async def get_tmate_session(container_name: str) -> dict:
    ...
    Returns dict:  {"ssh": "<ssh cmd>", "web": "<https://tmate.io/t/...>"}
    Raises Exception on failure so callers can treat it as optional.
    """
    # Verify tmate binary is present (pre-baked in image)
    ...
    # Kill any stale session then start a fresh one with a named socket
    start_script = r"""
...
"""
    ...
    for line in stdout.splitlines():
        ...
        elif line.startswith("TMATE_WEB:"):
            tmate_web = line[len("TMATE_WEB:"):].strip()

    if not tmate_ssh:
        raise Exception(f"tmate SSH string not returned. stderr: {stderr[:300]}")

    return {"ssh": tmate_ssh, "web": tmate_web}
...
async def get_sshx_session(container_name: str) -> str:
    ...
    Captures raw output, strips all ANSI escape sequences in Python (sshx emits
    colour codes that contaminate the URL when grepped naively), validates the
    URL format, and raises on any failure so callers can treat it as optional.
    """
    chk_out, _, chk_rc = await docker_exec(
...
sleep 1
# Capture raw output — ANSI stripping happens on the Python side
sshx > /tmp/sshx_raw.log 2>&1 &
# Wait up to 30 s for the URL to appear in the log
_waited=0
while [ $_waited -lt 30 ]; do
    if grep -q 'sshx.io' /tmp/sshx_raw.log 2>/dev/null; then
        break
    fi
    sleep 1
    _waited=$((_waited + 1))
...
"""
    ...
    # Extract the raw log section between our sentinels
    raw_log = ""
    in_raw = False
    for line in stdout.splitlines():
        if line.strip() == "SSHX_RAW_START":
            in_raw = True
            continue
        if line.strip() == "SSHX_RAW_END":
            in_raw = False
            continue
        if in_raw:
            raw_log += line + "\n"

    # Strip ALL ANSI escape sequences before searching for the URL
    clean_log = strip_ansi(raw_log)

    # Extract the sshx URL — must not contain whitespace or control chars
    url_match = re.search(r'https://sshx\.io/s/[^\s\x00-\x1f]+', clean_log)
    if not url_match:
        raise Exception(
            f"sshx URL not found in output.\n"
            f"Cleaned log: {clean_log[:400]}\n"
            f"Stderr: {stderr[:200]}"
        )

    url = url_match.group(0).strip()
    # Remove any trailing punctuation or control characters that slipped through
    url = re.sub(r'[\x00-\x1f\x7f-\x9f.]+$', '', url)

    # Strict format validation: must be https://sshx.io/s/<id>#<key>
    _SSHX_URL_RE = re.compile(r'https://sshx\.io/s/[A-Za-z0-9_\-]+#[A-Za-z0-9_\-]+')
    if not _SSHX_URL_RE.fullmatch(url):
        raise Exception(
            f'sshx URL format invalid after cleaning: {url!r}\n'
            f'Cleaned log: {clean_log[:300]}'
        )
            ...
            try:
                await interaction.followup.send(
                    embed=create_info_embed(
                        ...
                        "Starting tmate and sshx sessions, please wait…"
                    ),
                    ephemeral=True
                )

                # ── tmate SSH only (no web URL exposed to users) ───────────────
                tmate_info = {}
                tmate_err  = ""
                try:
                ...
                # Build access embed — tmate SSH string + sshx only
                ssh_embed = discord.Embed(
                    title="⬛  DarkNodes — VPS Access",
                    description=f"Active sessions for `{container_name}`:",
                    color=0x0d0d0d,
                    timestamp=datetime.utcnow()
                )
                ...
                if tmate_info.get("ssh"):
                    ...
                    ssh_embed.add_field(
                        name="🖥️  tmate SSH",
                        value=f"⚠️ Failed: `{tmate_err[:120]}`",
                        inline=False
                    )

                if sshx_url:
                    ssh_embed.add_field(name="🌐  sshx", value=sshx_url, inline=False)
                elif sshx_err:
                    ssh_embed.add_field(
                        name="🌐  sshx",
                        value=f"⚠️ Failed: `{sshx_err[:120]}`",
                        inline=False
                    )
                ...
                ssh_embed.add_field(
                    name="📌  Notes",
                    value=(
                        "• Sessions refresh each time you click SSH\n"
                        "• Use `!manage` to start, stop, or reinstall your VPS"
                    ),
                    inline=False
                )
                ssh_embed.set_footer(text="DarkNodes Infrastructure  •  Keep this safe")
                ...
                try:
                    ...
                    await interaction.followup.send(
                        embed=create_success_embed(
                            "Access Details Sent",
                            "Check your **DMs** for tmate SSH and sshx links."
                        ),
                        ephemeral=True
                    )
                except discord.Forbidden:
                    await interaction.followup.send(
                        embed=create_error_embed("DM Failed", "Enable DMs to receive access credentials!"),
                        ephemeral=True
                    )
            except Exception as e:
                await interaction.followup.send(
...
# ─── Deployment progress / DM embed helpers ───────────────────────────────────
...
def _build_deploy_progress_embed(current_step: str, container_name: str,
                                  ram, cpu, plan_label: str = "") -> discord.Embed:
    """Live deployment progress embed — edited in-place during provisioning."""
    try:
        idx = _DEPLOY_STEPS.index(current_step)
    except ValueError:
        idx = 0
    pct    = int((idx / len(_DEPLOY_STEPS)) * 100)
    filled = max(1, int(pct / 10))
    bar    = "█" * filled + "░" * (10 - filled)
    lines  = []
    for i, step in enumerate(_DEPLOY_STEPS):
        if   i < idx:  lines.append(f"✅  {step}")
        elif i == idx: lines.append(f"⏳  **{step}...**")
        else:          lines.append(f"◽  {step}")
    spec = f"**Container** `{container_name}`"
    if plan_label:
        spec += f"  ·  **Plan** {plan_label}"
    spec += f"  ·  **RAM** {ram}GB  ·  **CPU** {cpu} Core(s)"
    embed = discord.Embed(
        title="⬛  DarkNodes — Deploying VPS",
        description=f"```\n{bar}  {pct}%\n```\n" + "\n".join(lines),
        color=0x0d0d0d,
        timestamp=datetime.utcnow()
    )
    embed.add_field(name="\u200b", value=spec, inline=False)
    embed.set_footer(text="DarkNodes Infrastructure  •  Provisioning")
    return embed


def _build_deploy_done_embed(container_name: str, ram, cpu, vps_hostname: str,
                              vps_count: int, plan_label: str = "") -> discord.Embed:
    """'Deployment complete' embed that replaces the progress embed."""
    spec = f"**Container** `{container_name}`"
    if plan_label:
        spec += f"  ·  **Plan** {plan_label}"
    spec += f"  ·  **RAM** {ram}GB  ·  **CPU** {cpu} Core(s)"
    embed = discord.Embed(
        title="✅  DarkNodes — VPS Deployed",
        description=(
            f"**Node `{vps_hostname}`** is live and ready.\n"
            f"Check your **DMs** for access credentials."
        ),
        color=0x00ff88,
        timestamp=datetime.utcnow()
    )
    ...
    embed.add_field(
        name="▸  Manage",
        value="Use `!manage` → **Start · Stop · Reinstall · SSH**",
        inline=False
    )
    embed.set_footer(text=f"DarkNodes Infrastructure  •  VPS #{vps_count}  •  Deployed")
    return embed


def _build_dm_vps_embed(vps_count: int, ram, cpu, disk, container_name: str,
                         vps_hostname: str, tmate_info: dict, sshx_url: str,
                         plan_label: str = "", processor: str = "") -> discord.Embed:
    """
    Premium DarkNodes VPS-ready DM embed.
    Shows only tmate SSH string and sshx URL — no OpenSSH credentials exposed.
    """
    embed = discord.Embed(
        title="⬛  DarkNodes — VPS Ready",
        description=(
            f"**Node** `{vps_hostname}` is live and ready.\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ),
    ...
    ram_str  = f"{ram}GB"  if isinstance(ram, int)  else str(ram)
    cpu_str  = str(cpu)
    disk_str = f"{disk}GB" if isinstance(disk, int) else str(disk)
    spec_parts = []
    if plan_label:
        spec_parts.append(f"**Plan** {plan_label}")
    spec_parts += [f"**RAM** {ram_str}", f"**CPU** {cpu_str} Core(s)", f"**Disk** {disk_str}"]
    if processor:
        spec_parts.append(f"**Arch** {processor}")
    embed.add_field(name="▸  Specifications", value="  ·  ".join(spec_parts), inline=False)
    embed.add_field(name="▸  Container",      value=f"`{container_name}`  ·  Node `{vps_hostname}`", inline=False)

    access_parts = []
    if tmate_info.get("ssh"):
        access_parts.append(f"**🖥️  tmate SSH**\n```\n{tmate_info['ssh']}\n```")
    if sshx_url:
        access_parts.append(f"**🌐  sshx**\n{sshx_url}")

    if access_parts:
        embed.add_field(name="▸  Access Methods", value="\n\n".join(access_parts), inline=False)
    else:
        embed.add_field(
            name="▸  Access Methods",
            value=(
                "⚠️  No sessions available yet.\n"
                "Use `!manage` → **SSH** to generate tmate / sshx sessions."
            ),
            inline=False
        )
    embed.add_field(
        name="▸  Management",
        value="Use `!manage` → **Start · Stop · Reinstall · SSH** to manage your VPS.",
        inline=False
    )
    embed.set_footer(text=f"DarkNodes Infrastructure  •  VPS #{vps_count}  •  Keep this safe")
    return embed
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    # Send a single live-progress message that we'll edit through each stage
    progress_msg = await ctx.send(
        embed=_build_deploy_progress_embed("Building Docker image", container_name, ram, cpu)
    )

    async def on_progress(step: str):
        try:
            await progress_msg.edit(
                embed=_build_deploy_progress_embed(step, container_name, ram, cpu)
            )
        except Exception:
            pass
    ...
    try:
        await create_docker_container(
            container_name, ram_mb, cpu, ssh_port, password,
            disk_gb=disk, hostname=vps_hostname, progress_fn=on_progress
        )
        ...
        # ── Access methods ────────────────────────────────────────────────────
        await on_progress("Setting up access")
        ...
        # Edit the progress message → deployment complete
        await progress_msg.edit(
            embed=_build_deploy_done_embed(container_name, ram, cpu, vps_hostname, vps_count)
        )
        ...
        _log.set_author(name="VPS Created  •  Admin Deploy",
                        icon_url="https://cdn.discordapp.com/emojis/1044924827185127464.webp")
        try:
        ...
        _log.add_field(name="🌐 Status",       value="`🟢 RUNNING`",                 inline=True)
        _log.add_field(name="📅 Created At",
                       value=f"`{datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}`", inline=True)
        _log.set_footer(text="DarkNodes VPS Logs  •  Admin Deploy")
        asyncio.create_task(send_log(_log))

        # ── DM owner with premium access embed ────────────────────────────────
        try:
            await user.send(embed=_build_dm_vps_embed(
                vps_count, ram, cpu, disk, container_name,
                vps_hostname, tmate_info, sshx_url
            ))
        except discord.Forbidden:
            pass
        ...
        await progress_msg.edit(
            embed=create_error_embed("Creation Failed", f"Error: {str(e)[:800]}")
        )
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    # Send a single live-progress message that we'll edit through each stage
    ram_int = int(ram_str.replace("GB", ""))
    progress_msg = await ctx.send(
        embed=_build_deploy_progress_embed(
            "Building Docker image", container_name, ram_int, cpu_str,
            plan_label=f"{plan} ({processor})"
        )
    )
    ...
    async def on_progress(step: str):
        try:
            await progress_msg.edit(
                embed=_build_deploy_progress_embed(
                    step, container_name, ram_int, cpu_str,
                    plan_label=f"{plan} ({processor})"
                )
            )
        except Exception:
    ...
    try:
        await create_docker_container(
            ...
            hostname=vps_hostname, progress_fn=on_progress
        )
        ...
        # Edit the progress message → deployment complete
        await progress_msg.edit(
            embed=_build_deploy_done_embed(
                container_name, ram_int, cpu_str, vps_hostname, vps_count,
                plan_label=f"{plan} ({processor})"
            )
        )
        ...
        # ── DM owner with premium access embed ────────────────────────────────
        try:
            await ctx.author.send(embed=_build_dm_vps_embed(
                vps_count, ram_int, cpu_str, plans[plan]["storage"],
                container_name, vps_hostname, tmate_info, sshx_url,
                plan_label=plan, processor=processor
            ))
        except discord.Forbidden:
            pass
        ...
        try:
            ...
            pass
        await progress_msg.edit(
            embed=create_error_embed("Purchase Failed", f"Error: {str(e)[:800]}\n\nCredits refunded.")
        )
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    ...
    try:
        ...
        # Give systemd time to start services before checking SSH
        await asyncio.sleep(8)
        # Fix docker socket permissions and ensure SSH is running
        await docker_exec(
            container_name,
...
def _tmpl_progress_bar(pct: int) -> str:
    filled = max(1, int(pct / 10))
    return "█" * filled + "░" * (10 - filled)


def _build_tmpl_progress_embed(title: str, steps: list, step_idx: int,
                                 extra: str = "") -> discord.Embed:
    pct   = int((step_idx / max(len(steps), 1)) * 100)
    bar   = _tmpl_progress_bar(pct)
    lines = []
    for i, s in enumerate(steps):
        if   i < step_idx:  lines.append(f"✅  {s}")
        elif i == step_idx: lines.append(f"⏳  **{s}...**")
        else:               lines.append(f"◽  {s}")
    desc = f"```\n{bar}  {pct}%\n```\n" + "\n".join(lines)
    if extra:
        desc += f"\n\n{extra}"
    embed = discord.Embed(
        title=title, description=desc, color=0x0d0d0d, timestamp=datetime.utcnow()
    )
    embed.set_footer(text="DarkNodes  •  Template Installer")
    return embed


def _gen_token(n: int = 20) -> str:
    """Generate a URL-safe random string of length n."""
    return secrets.token_urlsafe(n)


def _gen_db_pass() -> str:
...
class TemplateVPSSelectorView(discord.ui.View):
    def __init__(self, user_id: str, vps_list: list):
        ...
        options = []
        for i, vps in enumerate(vps_list):
            name   = vps.get("hostname") or vps.get("container_name", f"VPS {i+1}")
            status = vps.get("status", "unknown").upper()
            ram    = vps.get("ram", "?")
            cpu    = vps.get("cpu", "?")
            options.append(discord.SelectOption(
                label=f"{name}  (VPS #{i+1})",
                description=f"Status: {status}  |  RAM: {ram}  |  CPU: {cpu} Core(s)",
                value=str(i),
                emoji="🖥️"
            ))

        sel = discord.ui.Select(
            placeholder="Select a VPS to install on…",
        ...
        self.initial_embed = discord.Embed(
            title="⬛  DarkNodes — Template Installer",
            description=(
                "Select the VPS you want to install a template on.\n\n"
                "> ⚠️  Installation runs **inside your VPS container**."
            ),
            color=0x0d0d0d
        )
        self.initial_embed.set_footer(text="DarkNodes  •  Template System")
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        if vps.get("status") != "running":
            await interaction.response.send_message(
                embed=create_error_embed(
                    "VPS Not Running",
                    f"The VPS must be running to install templates.\n"
                    f"Current status: `{vps.get('status','unknown')}`\n"
                    f"Start it with `!manage` first."
                ),
                ephemeral=True
            )
            return
        view  = TemplateSelectorView(self.user_id, vps)
        embed = discord.Embed(
            title="⬛  DarkNodes — Select Template",
            description=f"Installing on: `{vps['container_name']}`\n\nChoose a template:",
            color=0x0d0d0d
        )
        embed.add_field(name="🦖  Pterodactyl", value="Game server panel + Wings daemon", inline=True)
        embed.add_field(name="☁️  Cloudflare Tunnel", value="Secure tunnel to Cloudflare network", inline=True)
        embed.set_footer(text="DarkNodes  •  Template System")
...
class TemplateSelectorView(discord.ui.View):
    def __init__(self, user_id: str, vps: dict):
        super().__init__(timeout=180)
        self.user_id = user_id
        self.vps     = vps
        ...
        sel = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(
                    label="🦖  Pterodactyl",
                    description="Game server management panel + Wings daemon",
                    value="pterodactyl", emoji="🦖"
                ),
                discord.SelectOption(
                    label="☁️  Cloudflare Tunnel",
                    description="Connect your VPS to Cloudflare Zero Trust",
                    value="cloudflare", emoji="☁️"
                ),
            ],
    ...
    async def _on_select(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("This is not your session.", ephemeral=True)
            return
        tmpl = interaction.data["values"][0]
        if tmpl == "pterodactyl":
            view  = PterodactylTypeView(self.user_id, self.vps)
            embed = discord.Embed(
                title="🦖  Pterodactyl — What to Install?",
                description=f"Installing on: `{self.vps['container_name']}`",
                color=0x0d0d0d
            )
            embed.add_field(name="🖥️  Panel",  value="The Pterodactyl web panel\n*Requires a domain*", inline=True)
            embed.add_field(name="🪽  Wings",  value="The Wings daemon\n*Runs on a game server node*", inline=True)
            embed.set_footer(text="DarkNodes  •  Pterodactyl Template")
            await interaction.response.edit_message(embed=embed, view=view)
        elif tmpl == "cloudflare":
            await interaction.response.send_modal(CloudflareConfigModal(self.vps))


# ── Pterodactyl Type View ──────────────────────────────────────────────────────

class PterodactylTypeView(discord.ui.View):
    def __init__(self, user_id: str, vps: dict):
        super().__init__(timeout=180)
        self.user_id = user_id
        self.vps     = vps

    @discord.ui.button(label="🖥️  Install Panel", style=discord.ButtonStyle.primary)
    async def btn_panel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("This is not your session.", ephemeral=True)
            return
        await interaction.response.send_modal(PterodactylPanelModal(self.vps))

    @discord.ui.button(label="🪽  Install Wings", style=discord.ButtonStyle.secondary)
    async def btn_wings(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("This is not your session.", ephemeral=True)
            return
        await interaction.response.send_modal(PterodactylWingsModal(self.vps))


# ── Pterodactyl Panel Modal ────────────────────────────────────────────────────

class PterodactylPanelModal(discord.ui.Modal, title="🦖  Pterodactyl Panel Setup"):
    domain     = discord.ui.TextInput(label="Domain",     placeholder="panel.example.com", required=True, max_length=100)
    ssl_choice = discord.ui.TextInput(label="SSL (letsencrypt / http / custom)", placeholder="letsencrypt",
                                       required=True, max_length=20, default="letsencrypt")
    cert_path  = discord.ui.TextInput(label="Certificate path  (only for SSL=custom)",
                                       placeholder="/etc/ssl/certs/panel.crt", required=False, max_length=200)
    key_path   = discord.ui.TextInput(label="Private key path  (only for SSL=custom)",
                                       placeholder="/etc/ssl/private/panel.key", required=False, max_length=200)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        ssl    = self.ssl_choice.value.strip().lower()
        cert   = (self.cert_path.value  or "").strip()
        key    = (self.key_path.value   or "").strip()

        if ssl not in ("letsencrypt", "http", "custom"):
            await interaction.response.send_message(
                embed=create_error_embed("Invalid SSL", "SSL must be `letsencrypt`, `http`, or `custom`."),
                ephemeral=True
            )
            return
        if ssl == "custom" and (not cert or not key):
            await interaction.response.send_message(
                embed=create_error_embed("Missing SSL Paths", "Provide certificate and key paths for custom SSL."),
                ephemeral=True
            )
            return

        await interaction.response.send_message(
            embed=create_info_embed("🦖  Pterodactyl Panel", "Installation started — see progress below."),
            ephemeral=True
        )
        try:
            progress_msg = await interaction.channel.send(
                embed=_build_tmpl_progress_embed("🦖  Installing Pterodactyl Panel", PTERO_PANEL_STEPS, 0,
                                                  f"Domain: `{domain}`  ·  SSL: `{ssl}`")
            )
        except Exception:
            try:
                ...
                return
        asyncio.create_task(
            _install_pterodactyl_panel(interaction.user, self.vps, domain, ssl, cert, key, progress_msg)
        )


# ── Pterodactyl Wings Modal ────────────────────────────────────────────────────

class PterodactylWingsModal(discord.ui.Modal, title="🪽  Pterodactyl Wings Setup"):
    panel_url = discord.ui.TextInput(label="Panel URL", placeholder="https://panel.example.com",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=create_info_embed("🪽  Pterodactyl Wings", "Installation started — see progress below."),
            ephemeral=True
        )
        try:
            progress_msg = await interaction.channel.send(
                embed=_build_tmpl_progress_embed("🪽  Installing Pterodactyl Wings", PTERO_WINGS_STEPS, 0,
                                                  f"Panel: `{panel_url}`")
            )
        except Exception:
            try:
                ...
                return
        asyncio.create_task(
            _install_pterodactyl_wings(interaction.user, self.vps, panel_url, progress_msg)
        )


# ── Cloudflare Tunnel Modal ────────────────────────────────────────────────────

class CloudflareConfigModal(discord.ui.Modal, title="☁️  Cloudflare Tunnel Setup"):
    tunnel_name  = discord.ui.TextInput(label="Tunnel Name",   placeholder="my-vps-tunnel",
                                         required=True, max_length=100)
    tunnel_token = discord.ui.TextInput(label="Cloudflare Tunnel Token", placeholder="eyJhI…",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        tunnel_name  = self.tunnel_name.value.strip()
        tunnel_token = self.tunnel_token.value.strip()
        await interaction.response.send_message(
            embed=create_info_embed("☁️  Cloudflare Tunnel", "Installation started — see progress below."),
            ephemeral=True
        )
        try:
            progress_msg = await interaction.channel.send(
                embed=_build_tmpl_progress_embed("☁️  Installing Cloudflare Tunnel", CF_TUNNEL_STEPS, 0,
            ...
            try:
                ...
                return
        asyncio.create_task(
            _install_cloudflare_tunnel(interaction.user, self.vps, tunnel_name, tunnel_token, progress_msg)
        )


# ── Installation — Pterodactyl Panel ──────────────────────────────────────────

async def _install_pterodactyl_panel(owner_user, vps: dict, domain: str, ssl_type: str,
                                      cert_path: str, key_path: str, progress_msg):
    container_name = vps["container_name"]
    steps          = PTERO_PANEL_STEPS
    db_name        = f"ptero_{secrets.token_hex(4)}"
    db_user        = f"ptero_{secrets.token_hex(4)}"
    db_password    = _gen_db_pass()
    admin_user     = f"admin_{secrets.token_hex(3)}"
    admin_email    = f"admin_{secrets.token_hex(4)}@darknodes.local"
    admin_password = _gen_token(18)
    scheme         = "https" if ssl_type != "http" else "http"
    panel_url      = f"{scheme}://{domain}"

    async def _step(i: int, extra: str = ""):
        try:
            await progress_msg.edit(embed=_build_tmpl_progress_embed(
                "🦖  Installing Pterodactyl Panel", steps, i, extra
            ))
        except Exception:
            pass

    async def _fail(err: str):
        embed = discord.Embed(title="❌  Panel Installation Failed",
                               description=err[:1800], color=0xff3366, timestamp=datetime.utcnow())
        embed.add_field(name="Container", value=f"`{container_name}`", inline=True)
        embed.add_field(name="Retry", value="Fix the error then run `/template` again.", inline=False)
        embed.set_footer(text="DarkNodes  •  Installation Failed")
        try:
            await progress_msg.edit(embed=embed)
        except Exception:
            pass
        logger.error(f"Pterodactyl panel install failed for {container_name}: {err}")

    try:
        # Step 0 — base deps
        await _step(0)
        out, err, rc = await docker_exec(container_name, """
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq 2>&1 | tail -3
apt-get install -y --no-install-recommends software-properties-common apt-transport-https \
    ca-certificates curl lsb-release gnupg tar unzip git wget 2>&1 | tail -5
echo DEPS_OK
""", timeout=240)
        if "DEPS_OK" not in out:
            await _fail(f"Base dependency install failed:\n```{err[:500]}```")
            return
...
apt-get install -y --no-install-recommends \
    php8.3 php8.3-fpm php8.3-cli php8.3-bcmath php8.3-curl php8.3-gd php8.3-mbstring \
    php8.3-mysql php8.3-xml php8.3-zip php8.3-tokenizer php8.3-intl php8.3-readline 2>&1 | tail -5
echo PHP_OK
""", timeout=360)
        if "PHP_OK" not in out:
            await _fail(f"PHP 8.3 install failed:\n```{err[:500]}```")
            return

        # Step 2 — MariaDB
        await _step(2)
        out, err, rc = await docker_exec(container_name, f"""
export DEBIAN_FRONTEND=noninteractive
apt-get install -y --no-install-recommends mariadb-server 2>&1 | tail -5
systemctl enable mariadb 2>/dev/null || true
systemctl start mariadb 2>/dev/null || mariadbd --user=mysql &
sleep 6
mysql -u root <<'EOF'
...
echo MYSQL_OK
""", timeout=240)
        if "MYSQL_OK" not in out:
            await _fail(f"MariaDB setup failed:\n```{err[:500]}```")
            return

        # Step 3 — Redis
        await _step(3)
        await docker_exec(container_name, """
export DEBIAN_FRONTEND=noninteractive
apt-get install -y --no-install-recommends redis-server 2>&1 | tail -3
...
""", timeout=120)
        ...
        await docker_exec(container_name, """
export DEBIAN_FRONTEND=noninteractive
apt-get install -y --no-install-recommends nginx 2>&1 | tail -3
...
curl -fsSL -o panel.tar.gz "https://github.com/pterodactyl/panel/releases/latest/download/panel.tar.gz" 2>&1 | tail -3
tar -xzf panel.tar.gz --strip-components=1 && rm panel.tar.gz
chmod -R 755 storage/* bootstrap/cache/
echo DOWNLOAD_OK
""", timeout=360)
        if "DOWNLOAD_OK" not in out:
            await _fail(f"Panel download failed:\n```{err[:500]}```")
            return
...
curl -fsSL https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer 2>&1 | tail -3
composer install --no-dev --optimize-autoloader --no-interaction 2>&1 | tail -10
echo COMPOSER_OK
""", timeout=720)
        if "COMPOSER_OK" not in out:
            await _fail(f"Composer install failed:\n```{err[:500]}```")
            return
...
""", timeout=120)
        if "ENV_OK" not in out:
            await _fail(f"Environment setup failed:\n```{err[:500]}```")
            return

        # Step 8 — Migrations + admin user
        await _step(8)
        out, err, rc = await docker_exec(container_name, f"""
cd /var/www/pterodactyl
php artisan migrate --seed --force 2>&1 | tail -10
...
""", timeout=360)
        if "MIGRATE_OK" not in out:
            await _fail(f"Migration failed:\n```{err[:500]}```")
            return
        ...
        if ssl_type == "letsencrypt":
            nginx_conf = f"""
export DEBIAN_FRONTEND=noninteractive
apt-get install -y --no-install-recommends certbot python3-certbot-nginx 2>&1 | tail -3
...
certbot --nginx -d {domain} --non-interactive --agree-tos --email {admin_email} 2>&1 | tail -5
echo NGINX_OK
"""
...
echo NGINX_OK
"""
        await docker_exec(container_name, nginx_conf, timeout=300)

        # Step 10 — Services + queue worker
        await _step(10)
        await docker_exec(container_name, """
systemctl enable php8.3-fpm nginx 2>/dev/null || true
...
systemctl enable pteroq 2>/dev/null || true
systemctl start pteroq 2>/dev/null || true
chown -R www-data:www-data /var/www/pterodactyl
...
""", timeout=120)
        ...
        await asyncio.sleep(3)
        await docker_exec(container_name, """
systemctl is-active php8.3-fpm nginx mariadb redis-server 2>/dev/null || true
...
""", timeout=30)
        ...
        done_embed = discord.Embed(
            ...
            description=f"Panel is live at `{panel_url}`.\n📬 Credentials sent to your DMs.",
            color=0x00ff88, timestamp=datetime.utcnow()
        )
        done_embed.add_field(name="🌐  URL",       value=panel_url,              inline=True)
        done_embed.add_field(name="🖥️  Container", value=f"`{container_name}`",  inline=True)
        done_embed.set_footer(text="DarkNodes  •  Pterodactyl Panel")
        await progress_msg.edit(embed=done_embed)

        # DM credentials
        cred_embed = discord.Embed(
            title="🦖  Pterodactyl Panel — Credentials",
            description="Your panel has been installed. **Keep these credentials safe.**",
            color=0x0d0d0d, timestamp=datetime.utcnow()
        )
        cred_embed.add_field(name="🌐  Panel URL",   value=f"`{panel_url}`",     inline=False)
        cred_embed.add_field(name="👤  Username",    value=f"`{admin_user}`",    inline=True)
        cred_embed.add_field(name="📧  Email",       value=f"`{admin_email}`",   inline=True)
        cred_embed.add_field(name="🔑  Password",    value=f"||`{admin_password}`||", inline=True)
        cred_embed.add_field(name="🗄️  DB Name",    value=f"||`{db_name}`||",   inline=True)
        cred_embed.add_field(name="🗄️  DB User",    value=f"||`{db_user}`||",   inline=True)
        cred_embed.add_field(name="🗄️  DB Pass",    value=f"||`{db_password}`||", inline=True)
        cred_embed.add_field(
            name="⚠️  Security",
            value="• Change your admin password after first login\n"
                  "• Never share database credentials\n"
                  "• Keep this message safe",
            inline=False
        )
        cred_embed.set_footer(text="DarkNodes  •  Pterodactyl Panel  •  Keep this safe")
        try:
            await owner_user.send(embed=cred_embed)
        except discord.Forbidden:
            pass
...
# ── Installation — Pterodactyl Wings ──────────────────────────────────────────

async def _install_pterodactyl_wings(owner_user, vps: dict, panel_url: str, progress_msg):
    container_name = vps["container_name"]
    steps          = PTERO_WINGS_STEPS

    async def _step(i: int, extra: str = ""):
        try:
            await progress_msg.edit(embed=_build_tmpl_progress_embed(
                "🪽  Installing Pterodactyl Wings", steps, i, extra
            ))
        except Exception:
            pass

    async def _fail(err: str):
        embed = discord.Embed(title="❌  Wings Installation Failed",
                               description=err[:1800], color=0xff3366, timestamp=datetime.utcnow())
        embed.set_footer(text="DarkNodes  •  Installation Failed")
        try:
            await progress_msg.edit(embed=embed)
            ...
            pass
        logger.error(f"Wings install failed for {container_name}: {err}")

    try:
        # Step 0 — download Wings binary
        await _step(0)
        out, err, rc = await docker_exec(container_name, """
...
curl -fsSL -o /usr/local/bin/wings \
    "https://github.com/pterodactyl/wings/releases/latest/download/wings_linux_${SUFFIX}" 2>&1 | tail -3
chmod +x /usr/local/bin/wings
wings --version 2>&1 | head -1
echo WINGS_DL_OK
""", timeout=300)
        if "WINGS_DL_OK" not in out:
            await _fail(f"Wings download failed:\n```{err[:500]}```")
            return

        wings_ver = next((l.strip() for l in out.splitlines() if "wings" in l.lower() and "version" in l.lower()), "")

        # Step 1 — config dir
        await _step(1)
        await docker_exec(container_name, "mkdir -p /etc/pterodactyl", timeout=10)
...
""", timeout=30)
        ...
        await _step(3)
        ver_out, _, _ = await docker_exec(container_name, "wings --version 2>&1", timeout=15)

        # ── Success ──────────────────────────────────────────────────────────
        done_embed = discord.Embed(
            title="✅  Pterodactyl Wings Installed",
            description="Wings binary is installed. Configure it from your Panel.\n📬 Instructions sent to your DMs.",
            color=0x00ff88, timestamp=datetime.utcnow()
        )
        done_embed.add_field(name="🖥️  Container", value=f"`{container_name}`",          inline=True)
        done_embed.add_field(name="📋  Status",    value="`Awaiting config from panel`",  inline=True)
        done_embed.set_footer(text="DarkNodes  •  Pterodactyl Wings")
        await progress_msg.edit(embed=done_embed)

        instr_embed = discord.Embed(
            title="🪽  Pterodactyl Wings — Setup Guide",
            description="Wings is installed. Follow these steps to finish configuration:",
            color=0x0d0d0d, timestamp=datetime.utcnow()
        )
        instr_embed.add_field(
            name="1️⃣  Create a Node in your Panel",
            value=(
                f"• Go to `{panel_url}/admin/nodes/new`\n"
                "• Set **FQDN** to this server's domain or IP\n"
                "• **Daemon Port** `8080`  ·  **SFTP Port** `2022`\n"
                "• Save the node"
            ), inline=False
        )
        instr_embed.add_field(
            name="2️⃣  Get the config",
            value=(
                "• Node page → **Configuration** tab\n"
                "• Copy the `config.yml` content"
            ), inline=False
        )
        instr_embed.add_field(
            name="3️⃣  Apply and start",
            value=(
                "Paste into `/etc/pterodactyl/config.yml`, then:\n"
                "```\nsystemctl start wings\n```"
            ), inline=False
        )
        if wings_ver:
            instr_embed.add_field(name="📦  Wings Version", value=f"`{wings_ver}`", inline=True)
        instr_embed.add_field(name="🖥️  Container", value=f"`{container_name}`", inline=True)
        instr_embed.set_footer(text="DarkNodes  •  Pterodactyl Wings Setup")
        try:
            await owner_user.send(embed=instr_embed)
        except discord.Forbidden:
            pass
...
async def _install_cloudflare_tunnel(owner_user, vps: dict, tunnel_name: str,
                                      tunnel_token: str, progress_msg):
    container_name = vps["container_name"]
    steps          = CF_TUNNEL_STEPS

    async def _step(i: int, extra: str = ""):
        try:
            await progress_msg.edit(embed=_build_tmpl_progress_embed(
                "☁️  Installing Cloudflare Tunnel", steps, i, extra
            ))
        except Exception:
            pass

    async def _fail(err: str):
        embed = discord.Embed(title="❌  Tunnel Installation Failed",
                               description=err[:1800], color=0xff3366, timestamp=datetime.utcnow())
        embed.set_footer(text="DarkNodes  •  Installation Failed")
        try:
            await progress_msg.edit(embed=embed)
            ...
            pass
        logger.error(f"Cloudflare tunnel install failed for {container_name}: {err}")
...
curl -fsSL https://pkg.cloudflare.com/cloudflare-main.gpg \
    | gpg --dearmor -o /usr/share/keyrings/cloudflare-main.gpg 2>&1
echo 'deb [signed-by=/usr/share/keyrings/cloudflare-main.gpg] https://pkg.cloudflare.com/cloudflared jammy main' \
    | tee /etc/apt/sources.list.d/cloudflared.list
apt-get update -qq 2>&1 | tail -3
apt-get install -y cloudflared 2>&1 | tail -5
cloudflared --version 2>&1 | head -1
echo CF_INSTALLED
""", timeout=360)
        if "CF_INSTALLED" not in out:
            # Fallback: direct binary download
            out2, err2, rc2 = await docker_exec(container_name, """
...
echo CF_INSTALLED
""", timeout=300)
            if "CF_INSTALLED" not in out2:
                await _fail(f"cloudflared installation failed:\n```{(err or err2)[:500]}```")
                return
            cf_ver = next((l.strip() for l in out2.splitlines() if "cloudflared" in l.lower()), "")
        else:
            cf_ver = next((l.strip() for l in out.splitlines() if "cloudflared" in l.lower()), "")

        # Step 1 — systemd service
        await _step(1)
        await docker_exec(container_name, f"""
...
""", timeout=30)
        ...
        # Step 2 — start
        await _step(2)
        await docker_exec(container_name, "systemctl start cloudflared 2>/dev/null || true; sleep 5", timeout=30)

        # Step 3 — verify
        await _step(3)
        verify_out, _, _ = await docker_exec(container_name, """
sleep 3
systemctl is-active cloudflared 2>/dev/null | grep -q active && echo TUNNEL_ACTIVE || echo TUNNEL_STARTING
journalctl -u cloudflared --no-pager -n 5 2>/dev/null || true
""", timeout=30)
        tunnel_active = "TUNNEL_ACTIVE" in verify_out

        # ── Success ──────────────────────────────────────────────────────────
        status_str = "🟢  Running" if tunnel_active else "🟡  Starting (check in a moment)"
        done_embed = discord.Embed(
            title="✅  Cloudflare Tunnel Installed",
            description=f"Tunnel **{tunnel_name}** installed.\n📬 Configuration guide sent to your DMs.",
            color=0x00ff88, timestamp=datetime.utcnow()
        )
        done_embed.add_field(name="🖥️  Container", value=f"`{container_name}`", inline=True)
        done_embed.add_field(name="☁️  Status",    value=status_str,             inline=True)
        done_embed.set_footer(text="DarkNodes  •  Cloudflare Tunnel")
        await progress_msg.edit(embed=done_embed)

        guide_embed = discord.Embed(
            title="☁️  Cloudflare Tunnel — Configuration Guide",
            description=f"Tunnel **{tunnel_name}** is running. Add public hostnames in Cloudflare Zero Trust.",
            color=0x0d0d0d, timestamp=datetime.utcnow()
        )
        guide_embed.add_field(
            name="📍  How to configure",
            value="**Cloudflare Zero Trust → Networks → Tunnels → your tunnel → Public Hostnames → Add**",
            inline=False
        )
        guide_embed.add_field(
            ...
            value=(
                ...
                "**Service:** `HTTP`  ·  **URL:** `127.0.0.1`  ·  **Port:** `80`\n"
                "**Advanced → TLS Verify:** OFF"
            ), inline=False
        )
        guide_embed.add_field(
            ...
            value=(
                ...
                "**Service:** `HTTPS`  ·  **URL:** `127.0.0.1`  ·  **Port:** `443`\n"
                "**Advanced → TLS Verify:** OFF"
            ), inline=False
        )
        guide_embed.add_field(name="🖥️  Container", value=f"`{container_name}`",  inline=True)
        guide_embed.add_field(name="☁️  Tunnel",    value=f"`{tunnel_name}`",     inline=True)
        if cf_ver:
            guide_embed.add_field(name="📦  Version", value=f"`{cf_ver}`", inline=True)
        guide_embed.set_footer(text="DarkNodes  •  Cloudflare Tunnel Setup")
        try:
            await owner_user.send(embed=guide_embed)
        except discord.Forbidden:
            pass
...
@bot.tree.command(name="template",
                  description="Install software templates on your VPS (Pterodactyl, Cloudflare Tunnel…)")
async def template_command(interaction: discord.Interaction):
    """Interactive template installer: select VPS → choose template → follow prompts."""
    user_id  = str(interaction.user.id)
    vps_list = vps_data.get(user_id, [])
    ...
    if not vps_list:
        await interaction.response.send_message(
            embed=create_error_embed(
                "No VPS Found",
                "You don't have any VPS. Use `!buywc <plan>` to get one."
            ),
            ephemeral=True
        )
        return

    view = TemplateVPSSelectorView(user_id, vps_list)
