...
_DEFAULT_BOT_CONFIG: dict = {"logo_url": "", "name_slot_limit": 50}
...
def get_name_slot_limit() -> int:
    """Return how many numbered name slots to try before giving up (default: 50)."""
    return int(_bot_config.get("name_slot_limit", 50))

def set_name_slot_limit(limit: int) -> None:
    """Persist a new name-slot limit and update the in-memory copy."""
    _bot_config["name_slot_limit"] = limit
    _save_bot_config(_bot_config)
...
async def _unique_container_name(user_id: str, start_count: int) -> tuple:
    ...
    """
    count = start_count
    while count < start_count + get_name_slot_limit():
...
async def ensure_vps_image(force_rebuild: bool = False) -> tuple:
    ...
    async with _image_build_lock:
        ...
        async def _run_build(use_buildkit: bool) -> tuple:
            """Run docker build with or without BuildKit; return (returncode, logs)."""
        ...
        try:
            # Prefer BuildKit (=1) for parallel layers and better caching.
            # If the host is missing the buildx plugin, fall back to the legacy
            # builder (=0) automatically so the bot works on any Docker version.
            rc, build_logs = await _run_build(use_buildkit=True)
            if rc != 0 and any(kw in build_logs.lower() for kw in (
                "buildx", "component is missing", "unknown flag", "unknown command"
            )):
                logger.warning(
                    "BuildKit build failed (buildx likely missing) — "
                    "retrying with legacy builder (DOCKER_BUILDKIT=0) …"
                )
                rc, build_logs = await _run_build(use_buildkit=False)

            if rc != 0:
                # Log last 5000 chars so the failing layer/step is visible
                logger.error(
                    f"Image build FAILED (rc={rc}).\n"
                    f"Last build output:\n{build_logs[-5000:]}"
                )
...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None, progress_msg=None):
    ...
    # Resolve final container name — if this exact name is already taken by an
    # existing Docker container (e.g. from a previous failed deploy), skip to
    # the next free numbered slot instead of removing the old one.
    # Pattern: vps-{user_id}-{n}  →  try n, n+1, n+2 … until one is free.
    _name_parts = container_name.rsplit("-", 1)
    if len(_name_parts) == 2 and _name_parts[1].isdigit():
        _user_prefix = _name_parts[0]
        _start_n     = int(_name_parts[1])
        _n           = _start_n
        while _n < _start_n + get_name_slot_limit():
            _candidate = f"{_user_prefix}-{_n}"
            _, _, _rc  = await run_docker_command(f"docker inspect {_candidate}", timeout=10)
            if _rc != 0:   # non-zero = container doesn't exist = name is free
                if _candidate != container_name:
                    logger.warning(
                        f"Container name '{container_name}' is already taken — "
                        f"using '{_candidate}' instead."
                    )
                container_name = _candidate
                break
            logger.warning(f"Container name '{_candidate}' is taken — trying next slot.")
            _n += 1
    ...
    # Two-layer name conflict guard:
    #   Layer 1 (above): proactive docker inspect scan skips taken names before build.
    #   Layer 2 (here):  if Docker still returns a Conflict error at run time, find
    #                    the next free slot on the fly and retry — catches any race
    #                    condition or inspect false-negative on the host.
    await _progress(1)
    _run_attempts = 0
    _max_run_attempts = get_name_slot_limit()
    while True:
        _run_attempts += 1
        try:
            ...
            if "conflict" in err_str and "already in use" in err_str and _run_attempts < _max_run_attempts:
                # Name taken — advance to the next slot and rebuild run_cmd
                _name_parts2 = container_name.rsplit("-", 1)
                if len(_name_parts2) == 2 and _name_parts2[1].isdigit():
                    _next_n = int(_name_parts2[1]) + 1
                    _new_name = f"{_name_parts2[0]}-{_next_n}"
                    logger.warning(
                        f"[run] Conflict on '{container_name}' — retrying with '{_new_name}'."
                    )
                    container_name = _new_name
                    _vol_docker = f"{container_name}-docker"
                    _vol_home   = f"{container_name}-home"
                    _vol_root   = f"{container_name}-root"
                    _vol_opt    = f"{container_name}-opt"
                    ...
                    run_cmd = f"docker run -d {base_flags}{DOCKER_IMAGE}"
                    continue
                # Name doesn't match expected pattern — fall through to error
...
class ManageView(discord.ui.View):
    ...
    def add_action_buttons(self):
        # ── Row 0: Core controls ──────────────────────────────────────────────
        # Color logic: green=safe/start, red=destructive/stop, blue=info/connect, gray=utility
        ...
        cleanup_button = discord.ui.Button(label="🧹 Cleanup", style=discord.ButtonStyle.danger, row=0)
        ...
        # ── Row 1: Tools ─────────────────────────────────────────────────────
        fix_button = discord.ui.Button(label="🩺 Health Check", style=discord.ButtonStyle.primary, row=1)
        fix_button.callback = lambda inter: self.action_callback(inter, 'fix_scan')

        backup_button = discord.ui.Button(label="📸 Backup Now", style=discord.ButtonStyle.success, row=1)
        ...
        help_button = discord.ui.Button(label="❓ Get Help", style=discord.ButtonStyle.secondary, row=2)
        ...
        if action == 'reinstall':
            ...
            def _do_clone(src_name, target_vps_entry):
                ...
                async def callback(inter: discord.Interaction):
                    ...
                    try:
                        ...
                        await _docker_run_compat(run_cmd, timeout=120)
            ...
            try:
                ...
                class FileManagerView(discord.ui.View):
                    ...
                    @discord.ui.button(label="📜 Last System Logs", style=discord.ButtonStyle.secondary, row=1)
                                ...
                                if file_size_mb > 25:
                                    await inter.followup.send(
                                        embed=create_warning_embed("⚠️ Backup Too Large",
                                            ...
                                            f"The backup exists on the server. Transfer it via SCP:\n"
            ...
            _KB = [
                {
                    "keywords": ["ssh", "connect", "connection refused", "timeout", "can't ssh", "cannot ssh", "port 22"],
                    "title": "🔑 SSH Connection Issue",
                    "desc": "SSH service may not be running or the container needs a restart.",
                    "fix": "systemctl restart ssh 2>/dev/null; /usr/sbin/sshd 2>/dev/null || true; echo OK",
                ...
                {
                    "keywords": ["docker", "daemon", "dind", "docker not running", "docker failed", "cannot connect to docker"],
                    "title": "🐳 Docker Daemon Not Running",
                    "desc": "The Docker daemon inside your VPS may have stopped.",
                    "fix": "systemctl restart docker; sleep 4; docker info 2>&1 | head -5",
                    "fix_label": "Restart Docker Daemon",
                },
                {
                    "keywords": ["nginx", "apache", "web server", "port 80", "port 443", "site down", "website not loading"],
                    "title": "🌐 Web Server Down",
                    "desc": "Nginx or Apache may have crashed or a config error occurred.",
                    "fix": "nginx -t 2>&1; systemctl restart nginx 2>/dev/null; systemctl restart apache2 2>/dev/null; echo Done",
                ...
                },
                {
                    ...
                    "desc": "MySQL/MariaDB may have stopped or ran out of resources.",
                    "fix": "systemctl restart mysql 2>/dev/null; systemctl restart mariadb 2>/dev/null; systemctl status mysql 2>&1 | tail -5",
                    "fix_label": "Restart Database",
                },
                {
                    "keywords": ["high cpu", "cpu usage", "slow", "lag", "sluggish", "freezing", "overloaded"],
                    "title": "⚡ High CPU / Slowness",
                    "desc": "Check what's consuming resources.",
                    "fix": "ps aux --sort=-%cpu | head -15",
                    "fix_label": "Show Top Processes",
                },
                {
                    ...
                    "fix": "ping -c 3 8.8.8.8 2>&1; echo ---; nslookup google.com 2>&1 | head -5",
                ...
                },
                {
                    ...
                    "fix": "systemctl restart wings 2>/dev/null; systemctl status wings 2>&1 | tail -10",
                    "fix_label": "Restart Wings",
                },
                {
                    "keywords": ["memory", "ram", "oom", "killed", "out of memory", "swap"],
                    "title": "🧠 Low Memory / OOM",
                    "desc": "A process may have been killed due to low RAM.",
                    "fix": "free -h; echo ---; dmesg 2>/dev/null | grep -i 'oom\\|killed' | tail -5",
                    "fix_label": "Check Memory & OOM Logs",
                },
                {
                    "keywords": ["cloudflare", "tunnel", "cloudflared", "zero trust"],
                    "title": "☁️ Cloudflare Tunnel Issue",
                    "desc": "The cloudflared tunnel may have stopped running.",
                    "fix": "systemctl restart cloudflared 2>/dev/null; systemctl status cloudflared 2>&1 | tail -8",
            ...
            class HelpModal(discord.ui.Modal, title="🆘 Describe Your Issue"):
                issue = discord.ui.TextInput(
                    ...
                    placeholder="e.g. SSH not working, VPS is slow, disk full, Docker daemon stopped...",
                    required=True,
                    max_length=500,
                )
                ...
                async def on_submit(self_m, inter: discord.Interaction):
                    ...
                    best = [e for _, e in matched[:3]]

                    if not best:
                        no_match = create_info_embed(
                            "🆘 No Exact Match",
                            "Couldn't match your issue to a known fix. Try:\n"
                        ...
                        await inter.followup.send(embed=no_match, ephemeral=True)
                        return
                    ...
                    for i, entry in enumerate(best):
                        preview = entry['fix'][:80] + "…" if len(entry['fix']) > 80 else entry['fix']
                        ...
                        def _make_fix_cb(self_sv, entry):
                            async def callback(inter3: discord.Interaction):
                                ...
                                await inter3.followup.send(
                                    ...
                                    ephemeral=True)
                                try:
                                    out, err, rc = await docker_exec(_hs_container, entry['fix'], timeout=60)
                                    result_text = (out or err or "Done")[:700]
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        container_name = await create_docker_container(container_name, ram_mb, cpu, ssh_port, password, disk_gb=disk, hostname=vps_hostname, progress_msg=progress_msg)
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    ssh_port = get_next_ssh_port()
    try:
        container_name = await create_docker_container(container_name, ram_mb, cpu_str, ssh_port, password, hostname=vps_hostname, progress_msg=progress_msg)
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    ...
    try:
        ...
        await _docker_run_compat(run_cmd)
...
@bot.tree.command(name="setslotlimit", description="Set how many name slots the bot tries before giving up on a new VPS name (Admin only)")
@discord.app_commands.describe(limit="Number of slots to try (default: 50). Higher = more retries before failing.")
async def setslotlimit(interaction: discord.Interaction, limit: int):
    """Admin-only: configure the name-slot search limit used when a container name is taken."""
    user_id = str(interaction.user.id)
    if user_id != str(MAIN_ADMIN_ID) and user_id not in admin_data.get("admins", []):
        await interaction.response.send_message(
            embed=create_error_embed("Permission Denied", "Only admins can change this setting."),
            ephemeral=True,
        )
        return
    ...
    if limit < 1 or limit > 500:
        await interaction.response.send_message(
            embed=create_error_embed("Invalid Value", "Limit must be between 1 and 500."),
            ephemeral=True,
        )
        return

    old = get_name_slot_limit()
    set_name_slot_limit(limit)

    logo = get_logo_url()
    embed = discord.Embed(
        title="🔢  Name Slot Limit Updated",
        description=(
            f"When a container name is already taken, the bot will now try up to **{limit}** "
            f"numbered slots before giving up.\n\n"
            f"> Previous: `{old}`\n"
            f"> New: `{limit}`"
        ),
        color=get_embed_color(),
    ...
    embed.add_field(name="👤  Changed by", value=interaction.user.mention, inline=True)
    if logo:
        embed.set_footer(text=f"{get_brand_name()}  •  /setslotlimit", icon_url=logo)
    else:
        embed.set_footer(text=f"{get_brand_name()}  •  /setslotlimit")
    await interaction.response.send_message(embed=embed)
