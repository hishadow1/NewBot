...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None):
    ...
    Design:
    - The darknodes-vps image (built from the Dockerfile) ships with every
      package pre-installed and boots via /sbin/init (systemd as PID 1).
      No packages are installed at runtime — containers start in seconds.
    - --privileged + --cgroupns=host are required for systemd to manage
      cgroups inside the container.
    - tmpfs mounts on /run, /run/lock, /tmp satisfy systemd's runtime needs.
    - If any step fails the partial container is immediately stopped and
      removed, and a detailed error (including Docker startup logs) is raised.
      A broken or half-started VPS is never left running.
    """
    ...
    # 3. Start the container — CMD from the image is /sbin/init (systemd)
    ...
    try:
        ...
        await run_docker_command(f"docker rm -f {container_name}", timeout=15)
        raise RuntimeError(f"Failed to start container: {start_err}")

    # 4. Give systemd time to reach its running state before exec'ing into it
    await asyncio.sleep(5)

    # 5. Configure the VPS — password, hostname, verify SSH
    #    All packages are already baked into the image; this finishes in seconds.
    try:
        await configure_vps(container_name, password, hostname=_hostname)
    except Exception as cfg_err:
        startup_logs, _, _ = await run_docker_command(
            f"docker logs --tail=60 {container_name}", timeout=15
        )
        await run_docker_command(f"docker rm -f {container_name}", timeout=15)
        raise RuntimeError(
            f"VPS configuration failed: {cfg_err}\n\n"
            f"Container startup logs:\n{startup_logs or '(none)'}"
        )

    # 6. Strict verification — SSH must be running; on failure tear down cleanly
    passed, report = await verify_vps(container_name, hostname=_hostname)
    if not passed:
        startup_logs, _, _ = await run_docker_command(
            f"docker logs --tail=100 {container_name}", timeout=15
        )
        await run_docker_command(f"docker rm -f {container_name}", timeout=15)
        raise RuntimeError(
            f"VPS verification failed — container removed to prevent a broken deployment.\n\n"
...
async def configure_vps(container_name: str, password: str, hostname: str = None):
    ...
    The image already ships with all packages (SSH, systemd, tools).
    This function only:
      • Sets the root password (overrides the Dockerfile default "root")
      • Applies the unique per-container hostname
      • Ensures sshd is running (via systemd if available, else directly)
...
# Override the Dockerfile default password with the generated one
echo 'root:{password}' | chpasswd

# Apply unique per-container hostname
hostname {_hostname}
echo "{_hostname}" > /etc/hostname
grep -q "{_hostname}" /etc/hosts || echo "127.0.1.1 {_hostname}" >> /etc/hosts

# Ensure SSH host keys exist (already generated in image via ssh-keygen -A)
ssh-keygen -A 2>/dev/null || true

# Start / restart sshd — prefer systemd if PID 1 is systemd and running
if systemctl is-active --quiet ssh 2>/dev/null || systemctl is-active --quiet sshd 2>/dev/null; then
    : # SSH already active via systemd — nothing to do
    ...
    # systemd not running as PID 1 — start sshd directly
...
"""
    ...
    stdout, stderr, rc = await docker_exec(container_name, configure_script, timeout=60)
...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    Returns (critical_passed: bool, human_readable_report: str).
    Does NOT clean up on failure — the caller handles that.

    Critical checks (deployment fails if these do not pass):
      • SSH daemon is running

    Advisory checks (logged, displayed, but do not block deployment):
    ...
    async def run_check(label: str, cmd: str, expect: str = None, timeout: int = 15) -> bool:
        try:
            stdout, _, rc = await docker_exec(container_name, cmd, timeout=timeout)
            if rc != 0:
                results[label] = (False, f"exit {rc}")
                return False
            if expect and expect.lower() not in stdout.lower():
                results[label] = (False, f"'{expect}' not found — got: {stdout[:120]}")
                return False
    ...
    # ── Critical ──────────────────────────────────────────────────────────────
    ssh_ok = await run_check("SSH daemon", "pgrep -x sshd && echo ok", "ok")
    if not ssh_ok:
        # systemd may still be bringing services up — wait and retry once
        await asyncio.sleep(6)
        ssh_ok = await run_check("SSH daemon (retry)", "pgrep -x sshd && echo ok", "ok")

    # ── Advisory ─────────────────────────────────────────────────────────────
    await run_check("Hostname",    "hostname",                hostname or VPS_HOSTNAME)
    await run_check("systemctl",  "systemctl --version 2>&1", "systemd")
    await run_check("Docker CLI", "docker --version 2>&1",    "Docker")

    # Build human-readable report
    lines = [
        f"{'✅' if ok else '❌'} {lbl}: {detail}"
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        _log = discord.Embed(
                            ...
                            description=f"{interaction.user.mention} reinstalled VPS `{self.container_name}`.",
                        ...
                        _log.set_footer(text="DarkNodes VPS Logs")
            ...
            try:
                ...
                _log = discord.Embed(
                    title="▶️  VPS Started",
                    description=f"{interaction.user.mention} started VPS `{container_name}`.",
            ...
            try:
                ...
                _log = discord.Embed(
                    title="⏹️  VPS Stopped",
                    description=f"{interaction.user.mention} stopped VPS `{container_name}`.",
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        ...
        _log = discord.Embed(
            title="🟢  VPS Created",
            description=f"A new VPS was deployed by {ctx.author.mention} for {user.mention}.",
            colour=discord.Colour.from_rgb(0, 210, 100),
            timestamp=datetime.utcnow()
        )
        _log.add_field(name="📦 Container",   value=f"`{container_name}`",           inline=True)
        _log.add_field(name="🏷️ Hostname",    value=f"`{vps_hostname}`",             inline=True)
        _log.add_field(name="🆔 VPS ID",      value=f"`#{vps_count}`",               inline=True)
        _log.add_field(name="👤 Owner",        value=f"{user.mention} (`{user.id}`)", inline=True)
        _log.add_field(name="🛠️ Deployed By",  value=ctx.author.mention,              inline=True)
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    try:
        ...
        _log = discord.Embed(
            title="🔴  VPS Deleted",
            description=f"{ctx.author.mention} deleted VPS **#{vps_number}** belonging to {user.mention}.",
        ...
        _log.add_field(name="📝 Reason",     value=reason,                          inline=False)
        _log.set_footer(text="DarkNodes VPS Logs")
        asyncio.create_task(send_log(_log))
    except Exception as e:
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    try:
        ...
        _log = discord.Embed(
            title="🟢  VPS Created (Purchase)",
            description=f"{ctx.author.mention} purchased a **{plan}** ({processor}) VPS.",
            colour=discord.Colour.from_rgb(0, 210, 100),
            timestamp=datetime.utcnow()
        )
        _log.add_field(name="📦 Container",  value=f"`{container_name}`",                inline=True)
        _log.add_field(name="🏷️ Hostname",   value=f"`{vps_hostname}`",                  inline=True)
        _log.add_field(name="🆔 VPS ID",     value=f"`#{vps_count}`",                    inline=True)
        _log.add_field(name="👤 Owner",       value=f"{ctx.author.mention} (`{user_id}`)",inline=True)
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    ...
    try:
        ...
        # /sbin/init, so we do NOT override it with sleep infinity here.
        run_cmd = (
            f"docker run -d "
            f"--name {container_name} "
        ...
        await asyncio.sleep(5)
        # SSH is managed by systemd in the snapshot; direct start is a fallback
...
async def vps_suspend(ctx, user: discord.Member, vps_number: int, *, reason: str = "Manual admin suspension"):
    ...
    _log = discord.Embed(
        title="⛔  VPS Suspended (Manual)",
        description=f"VPS **#{vps_number}** belonging to {user.mention} was manually suspended.",
        colour=discord.Colour.from_rgb(220, 20, 60),
        timestamp=datetime.utcnow()
    )
    _log.add_field(name="📦 Container", value=f"`{container_name}`",            inline=True)
...
async def vps_unsuspend(ctx, user: discord.Member, vps_number: int):
    ...
    _log = discord.Embed(
        ...
        description=f"VPS **#{vps_number}** belonging to {user.mention} has been restored.",
        colour=discord.Colour.from_rgb(0, 210, 100),
        timestamp=datetime.utcnow()
    )
    _log.add_field(name="📦 Container", value=f"`{container_name}`",            inline=True)
...
async def setlogchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    embed.set_footer(text="DarkNodes  •  /setlogchannel")
