...
def create_embed(title, description="", color=0x5865F2, fields=None):
    ...
    if logo:
        embed.set_author(name="DarkNodes VPS Hosting", icon_url=logo)
        embed.set_thumbnail(url=logo)
    if fields:
        for field in fields:
            embed.add_field(name=field['name'], value=field["value"], inline=field.get("inline", False))
    kw: dict = {"text": "DarkNodes  •  High-Performance VPS Hosting"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
def _deploy_progress_embed(step_index: int, steps: list = None, failed: bool = False) -> discord.Embed:
    ...
    title = "❌  Deploy Failed" if failed else "🚀  DarkNodes VPS — Deploying"
    logo  = get_logo_url()
    embed = discord.Embed(
        title=title,
...
def _deploy_success_embed(user: discord.Member, vps_count: int, container_name: str,
    ...
    embed = discord.Embed(
        ...
        description=(
            f"**{user.mention}** — your VPS is online and ready.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🧠 **{ram} GB** RAM  ·  ⚙️ **{cpu}** vCPU  ·  💾 **{disk} GB** Disk\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━"
        ),
        color=0x23272A,
        timestamp=_utcnow(),
    ...
    embed.add_field(
        name="📩  Credentials",
        value="Check your **DMs** — login details & session links have been sent.",
        inline=False,
    )
    kw: dict = {"text": "DarkNodes VPS  •  Deployed & Ready"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
def _vps_dm_embed(vps_count: int, container_name: str, ram: str, cpu: str,
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Welcome to **DarkNodes**! Your server is online and ready.\n"
    ...
    if tmate_ssh or sshx_url:
        ...
        if sshx_url:
            access_parts.append(f"**🔗 sshx Web Terminal**\n{sshx_url}")
        embed.add_field(name="🔑  Access Methods", value="\n".join(access_parts), inline=False)
    else:
        embed.add_field(
            name="🔑  Access Methods",
            value=(
                "> Run `!manage` → click **SSH** to generate your live session links.\n"
                "> Links refresh every time — no password needed."
            ),
            inline=False,
        )
    ...
    kw: dict = {"text": "DarkNodes VPS  •  Keep this DM safe — do not share"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
async def dind_autoheal(container_name: str, delay: int = 180, notify_user: discord.Member = None):
    ...
    if await _dind_ok():
        ...
        if logo:
            embed.set_author(name="DarkNodes VPS — DinD Status", icon_url=logo)
        embed.add_field(name="📦 Container", value=f"`{container_name}`", inline=True)
        embed.add_field(name="⏱️ Confirmed After", value=f"`{delay}s`", inline=True)
        embed.add_field(name="🐳 DinD", value="✅ Healthy", inline=True)
        if logo:
            embed.set_footer(text="DarkNodes VPS  •  Auto-Check", icon_url=logo)
        ...
        if notify_user:
            try:
                ...
                if logo:
                    dm_embed.set_author(name="DarkNodes VPS Hosting", icon_url=logo)
                    dm_embed.set_footer(text="DarkNodes VPS  •  Auto-Check", icon_url=logo)
    ...
    if healed:
        ...
        if logo:
            embed.set_author(name="DarkNodes VPS — Auto-Heal", icon_url=logo)
        embed.add_field(name="📦 Container",    value=f"`{container_name}`",     inline=True)
        ...
        if logo:
            embed.set_footer(text="DarkNodes VPS  •  Auto-Heal", icon_url=logo)
        await send_log(embed)
    else:
...
# Also try add-apt-repository (works on both formats, no-ops if already enabled)
...
# ── 2h. apt update — run properly now that DNS and sources are set ─────────────
DEBIAN_FRONTEND=noninteractive apt-get update -qq 2>&1 | tail -5 || \
    DEBIAN_FRONTEND=noninteractive apt-get update --fix-missing -qq 2>/dev/null || true
...
#!/bin/bash
# DarkNodes VPS welcome banner — shown on every interactive login
...
echo "  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚══════╝"
echo ""
echo "  Welcome to your DarkNodes VPS"
echo "  ─────────────────────────────────────────────────────────────"
printf "  %-16s %s\n" "Hostname:"   "$_hn"
printf "  %-16s %s\n" "OS:"         "$_os"
printf "  %-16s %s\n" "Kernel:"     "$_kern"
...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    # fuse-overlayfs needs longer to initialise than overlay2 — give systemd
    # extra time before the first service check.
    ...
    # Poll until dockerd socket actually responds (up to 120 s).
    # "service active" only means the process started — the socket may not be
    # accepting connections yet while fuse-overlayfs mounts /var/lib/docker.
    # Each `docker info` attempt is bounded to 4 s so one hung call cannot
    # consume the entire window.  At 60 s we force-restart docker in case it
    # entered a crash-restart cycle.
    docker_info_ok = await run_check(
        "Docker daemon info",
        (
            r"bash -c '"
            r"for i in $(seq 1 120); do "
            ...
            r"  sleep 1; "
            r"done; "
            r"echo \"Timed out — dockerd not ready after 120s:\"; "
    ...
    _OPTIONAL = frozenset({
        "Docker daemon info",   # DinD can take >120s on first fuse-overlayfs mount
        "docker ps",
...
async def get_sshx_session(container_name: str) -> str:
    ...
    if chk_rc != 0:
        ...
        install_out, install_err, install_rc = await docker_exec(
            container_name,
            "curl -sSf https://sshx.io/get | sh -s -- -y 2>&1",
            timeout=60,
        )
    ...
    if channel_id:
        try:
            ...
            if channel:
                ...
                _logo = get_logo_url()
                _footer_kw: dict = {"text": "DarkNodes Anti-Abuse System  •  Auto-Monitor"}
                if _logo:
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Reinstall", **({"icon_url": _rl} if _rl else {})})
            ...
            try:
                ...
                _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Start Event", **({"icon_url": _logo_url} if _logo_url else {})})
                asyncio.create_task(send_log(_log))
            except Exception as e:
                await interaction.followup.send(
            ...
            try:
                ...
                _log.add_field(name="🧠 RAM",        value=f"`{vps.get('ram', 'N/A')}`",        inline=True)
                _log.add_field(name="⚙️ CPU",        value=f"`{vps.get('cpu', 'N/A')} Core(s)`", inline=True)
                _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Stop Event", **({"icon_url": _stop_logo} if _stop_logo else {})})
                asyncio.create_task(send_log(_log))
            except Exception as e:
                await interaction.followup.send(
            ...
            try:
                ...
                if _ssh_logo:
                    ssh_embed.set_author(name="DarkNodes VPS Hosting", icon_url=_ssh_logo)
                ...
                _ssh_fkw: dict = {"text": "DarkNodes VPS  •  Keep these links private"}
                if _ssh_logo:
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        ...
        _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Admin Deploy", **({"icon_url": _create_logo} if _create_logo else {})})
        asyncio.create_task(send_log(_log))
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    try:
        ...
        _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Delete Event", **({"icon_url": _del_logo} if _del_logo else {})})
        asyncio.create_task(send_log(_log))
    except Exception as e:
...
async def server_stats(ctx):
    ...
    embed = create_embed("📊  Server Statistics", "Live overview of the DarkNodes infrastructure.", 0x9B59B6)
...
async def vps_suspend(ctx, user: discord.Member, vps_number: int, *, reason: str = "Manual admin suspension"):
    ...
    _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Manual Suspension", **({"icon_url": _susp_logo} if _susp_logo else {})})
    asyncio.create_task(send_log(_log))
...
async def vps_unsuspend(ctx, user: discord.Member, vps_number: int):
    ...
    _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Unsuspend Event", **({"icon_url": _unsusp_logo} if _unsusp_logo else {})})
    asyncio.create_task(send_log(_log))
...
async def setlogo(interaction: discord.Interaction, url: str):
    ...
    embed = discord.Embed(
        ...
        description=(
            "The DarkNodes logo has been updated across **all embeds**.\n"
    ...
    embed.set_footer(text="DarkNodes  •  /setlogo", icon_url=logo)
