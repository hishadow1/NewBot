...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    Critical (deployment fails if any fail):
      ...
      ✓ systemctl is-active ssh  (NOTE: is-system-running "maintenance" is fine)
    ...
    # Use systemctl is-active — works regardless of is-system-running state
    ssh_ok = await run_check("SSH active",
    ...
    if not ssh_ok:
        ...
        await run_check("SSH active (retry)",
                        "systemctl is-active ssh 2>/dev/null || "
                        "systemctl is-active sshd 2>/dev/null || echo inactive",
                        "active")
    ...
    docker_ver_ok = await run_check("docker version",
                                    "docker version 2>&1", "Server")
    docker_ps_ok  = await run_check("docker ps",
                                    "docker ps --format '{{.ID}}' 2>&1", None)
    docker_img_ok = await run_check("docker images",
                                    "docker images --format '{{.Repository}}' 2>&1", None)

    if not (docker_ver_ok and docker_ps_ok and docker_img_ok):
        await asyncio.sleep(3)
        if not docker_ver_ok:
            await run_check("docker version (retry)", "docker version 2>&1", "Server")
        if not docker_ps_ok:
            await run_check("docker ps (retry)",
                            "docker ps --format '{{.ID}}' 2>&1", None)
        if not docker_img_ok:
            await run_check("docker images (retry)",
                            "docker images --format '{{.Repository}}' 2>&1", None)

    # docker run hello-world confirms end-to-end daemon access
    await run_check("docker run hello-world",
                    "docker run --rm hello-world 2>&1 | tail -5",
                    "Hello from Docker", timeout=60)
    ...
    _OPTIONAL = frozenset({
        ...
        "docker run hello-world", "tmate installed", "sshx installed",
    })
