                ...
                def _make_clone_cb(self_cv, target_idx, target_vps):
                    async def callback(inter: discord.Interaction):
                        ...
                        try:
                            ...
                            # Cleanup temp image
                            try:
                                await execute_docker(f"docker rmi {clone_image}", timeout=30)
                            except Exception:
                                pass
                ...
                class BackupListView(discord.ui.View):
                    def __init__(bv_self):
                        ...
                        for b_name, b_size, b_created in backups[:4]:
                            ...
                            dl_btn.callback = bv_self._make_dl_cb(b_name)
                            bv_self.add_item(dl_btn)

                    def _make_dl_cb(bv_self, image_name):
                        async def callback(inter: discord.Interaction):
        ...
        class BackupDownloadView(discord.ui.View):
            def __init__(bv):
                ...
                for b_name, b_size, b_created in backups[:4]:
                    ...
                    dl_btn = discord.ui.Button(label=f"⬇ {short}", style=discord.ButtonStyle.primary)
                    dl_btn.callback = bv._make_dl(b_name)
                    bv.add_item(dl_btn)

            def _make_dl(bv, image_name):
                async def callback(interaction: discord.Interaction):
    ...
    class CloneConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, btn: discord.ui.Button):
            ...
            try:
                ...
                try:
                    await execute_docker(f"docker rmi {clone_image} 2>/dev/null", timeout=15)
                except Exception:
                    pass
...
async def files_cmd(ctx, vps_number: int = 1, *, path: str = "/root"):
    ...
    ls_out, ls_err, rc = await docker_exec(container_name, f"ls -lhA {safe_path} 2>&1 | head -40", timeout=15)
    if rc != 0 or "No such file or directory" in ls_out:
        await ctx.send(embed=create_error_embed("Path Not Found", f"Directory `{path}` does not exist."))
        return
    lines = [l for l in ls_out.splitlines() if l.strip() and not l.startswith("total")]
...
async def edit_file_cmd(ctx, vps_number: int, *, file_path: str):
    ...
    container_name = vps["container_name"]
    safe_fp = shlex.quote(file_path)
    out, err, rc = await docker_exec(container_name, f"cat {safe_fp} 2>&1 | head -80", timeout=15)
    if rc != 0 or (not out.strip() and "No such file" in err):
        await ctx.send(embed=create_error_embed("File Not Found", f"`{file_path}` does not exist."))
        return
    ...
    class EditView(discord.ui.View):
        def __init__(self_ev):
            super().__init__(timeout=300)

        @discord.ui.button(label="⬇ Download File", style=discord.ButtonStyle.primary)
        async def dl_btn(self_ev, interaction: discord.Interaction, btn: discord.ui.Button):
            if str(interaction.user.id) != user_id and not is_user_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Not your VPS!"), ephemeral=True)
                return
            await interaction.response.defer(ephemeral=True)
