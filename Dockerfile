...
# ── apt reliability + non-interactive defaults ────────────────────────────────
...
# ── All stable packages in ONE layer ─────────────────────────────────────────
# One RUN = one vfs layer copy instead of 18. No inline backtick comments here
# (they cause shell expansion noise); groups are separated by blank lines only.
# Heavy/version-uncertain packages (Java, Rust apt, redis-tools, mysql-client)
# are in the soft-fail block below so a renamed package never kills the whole build.
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    && for b in \
         ...
         /usr/sbin/tcpdump \
         /usr/bin/nmap \
         /usr/bin/mtr   /usr/sbin/mtr-packet; do \
         setcap -r "$b" 2>/dev/null || true; \
       done \
    && apt-get clean && rm -rf /var/lib/apt/lists/*

# ── Soft-fail packages ────────────────────────────────────────────────────────
# Packages that may be renamed, missing, or very heavy on some Ubuntu versions.
# Semicolons let each attempt run independently — one failure never skips the rest.
RUN apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y neovim        2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y iftop dstat glances 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y iftop dstat    2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y fastfetch inxi hwinfo 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y inxi           2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y python-is-python3 2>/dev/null \
   || ln -sf /usr/bin/python3 /usr/local/bin/python \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y pipx           2>/dev/null || true \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y default-jdk default-jre 2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y rustc cargo    2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y sqlite3 redis-tools 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y sqlite3 valkey-tools 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y sqlite3        2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y default-mysql-client 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y mariadb-client 2>/dev/null || true \
    ; DEBIAN_FRONTEND=noninteractive apt-get install -y postgresql-client 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y postgresql-client-16 2>/dev/null \
   || DEBIAN_FRONTEND=noninteractive apt-get install -y postgresql-client-17 2>/dev/null || true \
    ; apt-get clean && rm -rf /var/lib/apt/lists/*
...
# Kept lean on purpose — scipy/matplotlib/jupyter/django are hundreds of MB and
# users can install them with a single `pip install` inside the VPS. Only install
# packages that are genuinely useful across nearly every Python project.
RUN pip3 install --no-cache-dir \
    requests httpx flask fastapi uvicorn gunicorn \
    sqlalchemy alembic psycopg2-binary pymysql \
    redis pydantic rich typer click \
...
# numpy + pandas only — scipy/matplotlib/jupyter are too large for a base image
RUN pip3 install --no-cache-dir numpy pandas 2>/dev/null || true
...
RUN mkdir -p /etc/needrestart/conf.d && \
    ...
    # Populate command-not-found handler DB
...
# ── tmate (static binary — works on all Ubuntu versions) ─────────────────────
RUN set -eux; \
    arch="$(uname -m)"; \
    case "$arch" in \
