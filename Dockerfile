...
# Consolidating into a single RUN block cuts peak disk usage during the build:
# the vfs storage driver copies the entire layer filesystem for each RUN, so
# 18 separate blocks would hold 18 full copies on disk simultaneously. One block
# = one copy. Every group of packages is labelled with a comment.
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    perl golang \
    \
    `# ── Rust (apt — rustup installed later for latest stable) ──────` \
    rustc cargo \
    \
    `# ── Java ───────────────────────────────────────────────────────` \
    ...
    \
    # Strip security.capability xattrs from network binaries.
    # The vfs storage driver copies layers with `cp -a`; if any binary carries
    # a security.capability xattr and the host fs rejects it, Docker fails with
    # "invalid argument" when preparing the next layer.
    && for b in \
...
# ── Soft-fail packages (may not exist on all Ubuntu versions) ─────────────────
# Each install is independent; semicolons ensure later installs always run even
# if an earlier one fails. All cleanup happens once at the end.
RUN apt-get update \
    `# neovim — not in Ubuntu 26.04 main, present in universe` \
   ...
   || ln -sf /usr/bin/python3 /usr/local/bin/python \
    ...
    `# postgresql client — try versioned names as fallback` \
