...
RUN arch="$(uname -m)" \
    && case "$arch" in \
        ...
        aarch64) ta="arm64v8" ;; \
        armv7l) ta="arm32v7" ;; \
        *) echo "unsupported arch for tmate: $arch" >&2; exit 1 ;; \
    esac \
    && curl -fsSL "https://github.com/tmate-io/tmate/releases/download/2.4.0/tmate-2.4.0-static-linux-${ta}.tar.xz" -o /tmp/tmate.tar.xz \
    && tar -xf /tmp/tmate.tar.xz -C /tmp \
    && TMATE_DIR="$(find /tmp -maxdepth 1 -type d -name 'tmate-*' | head -n1)" \
    && test -n "$TMATE_DIR" \
    && install -m 755 "$TMATE_DIR/tmate" /usr/local/bin/tmate \
    && rm -rf /tmp/tmate* "$TMATE_DIR" \
    && tmate -V
