...
RUN arch="$(uname -m)" && \
    ...
    curl --retry 5 --retry-delay 5 --retry-max-time 120 -fsSL \
...
# The pipe script adds a full round-trip to sshx.io and can hang on slow
# networks; a direct binary download is faster and more reliable.
# ─────────────────────────────────────────────────────────────────────────────
RUN set -eux; \
    arch="$(uname -m)"; \
    case "$arch" in \
        ...
        *) echo "unsupported arch for sshx: $arch" >&2; exit 1 ;; \
    esac && \
    curl --retry 3 --retry-max-time 30 -fsSL \
