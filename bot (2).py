...
# Base hostname prefix.  Falls back to "DarkNodes" if brand not yet loaded.
# get_next_vps_hostname() is called at runtime so it always picks up the live brand name.
VPS_HOSTNAME = "DarkNodes"  # kept for legacy references; prefer get_brand_name()

def get_next_vps_hostname() -> str:
    """Return a unique hostname like <BrandName>-1, <BrandName>-2, … for a new container.
    Always uses the current brand name so /branding changes apply to future deploys."""
    # Sanitise brand name: only keep alphanumerics and hyphens (valid hostname chars).
    import re as _re
    raw   = get_brand_name() if "get_brand_name" in dir() else VPS_HOSTNAME
    safe  = _re.sub(r"[^A-Za-z0-9-]", "", raw) or VPS_HOSTNAME
    total = sum(len(v) for v in vps_data.values())
    return f"{safe}-{total + 1}"
...
# Persisted in bot_config.json; admins update it via slash commands.

_BOT_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot_config.json")
# Per-embed colour keys:
#   deploy_progress  — in-progress bar colour (not used for failed/complete states)
#   deploy_success   — "Server Online" channel embed
#   dm               — "Your VPS is Live!" DM embed
#   general          — fallback / all other embeds  (also the legacy embed_color key)
_DEFAULT_BOT_CONFIG: dict = {
    "logo_url": "",
    "brand_name": "DarkNodes",
    "embed_color": 0x000000,          # legacy key — still used as "general" fallback
    "embed_colors": {
        "deploy_progress": 0x000000,
        "deploy_success":  0x000000,
        "dm":              0x000000,
        "general":         0x000000,
    },
}
...
def get_embed_color() -> int:
    """Return the general sidebar accent color for bot embeds (default: black)."""
    return _bot_config.get("embed_color", 0x000000)

def set_embed_color(color: int) -> None:
    """Persist a new general embed sidebar color and update the in-memory copy."""
    _bot_config["embed_color"] = color
    _save_bot_config(_bot_config)

# Per-embed-type color helpers
# Valid embed_type values: "deploy_progress", "deploy_success", "dm", "general"
def get_embed_color_for(embed_type: str) -> int:
    """Return the color for a specific embed type, falling back to the general color."""
    colors = _bot_config.get("embed_colors", {})
    if embed_type in colors:
        return colors[embed_type]
    return get_embed_color()

def set_embed_color_for(embed_type: str, color: int) -> None:
    """Persist a color for a specific embed type."""
    if "embed_colors" not in _bot_config:
        _bot_config["embed_colors"] = {}
    _bot_config["embed_colors"][embed_type] = color
    # Keep legacy embed_color in sync when setting "general"
    if embed_type == "general":
        _bot_config["embed_color"] = color
    _save_bot_config(_bot_config)

def set_all_embed_colors(color: int) -> None:
    """Set all per-embed colors at once (used by 'All Embeds' option)."""
    _bot_config["embed_color"] = color
    _bot_config["embed_colors"] = {
        "deploy_progress": color,
        "deploy_success":  color,
        "dm":              color,
        "general":         color,
    }
    _save_bot_config(_bot_config)
...
async def _update_container_brand(container_name: str, brand: str) -> None:
    """Write the brand name and updated hostname into a running container.
    
    Updates /etc/darknodes-brand (MOTD) and the container hostname, keeping
    the existing numeric suffix (e.g. -1) while swapping the brand prefix.
    """
    try:
        import re as _re
        safe_host = _re.sub(r"[^A-Za-z0-9-]", "", brand) or "DarkNodes"
        safe_brand = brand.replace("'", "'\\''")  # shell-safe single-quoted string
        # Use printf to write brand file; use sed+hostname to update the hostname.
        # We keep the existing -N suffix to avoid renumbering.
        script_lines = [
            f"printf '%s' '{safe_brand}' > /etc/darknodes-brand",
            "OLD=$(cat /etc/hostname 2>/dev/null | tr -d '\\n' || echo '1')",
            "SUFFIX=$(echo \"$OLD\" | grep -oE '[0-9]+$' || echo '1')",
            "SUFFIX=${SUFFIX:-1}",
            f"NEW='{safe_host}-'\"$SUFFIX\"",
            "hostname \"$NEW\"",
            "echo \"$NEW\" > /etc/hostname",
            "grep -q \"$NEW\" /etc/hosts || echo \"127.0.1.1 $NEW\" >> /etc/hosts",
        ]
        ...
        pass   # silent — offline containers will pick up the new hostname on next deploy
...
def _deploy_progress_embed(step_index: int, steps: list = None, failed: bool = False) -> discord.Embed:
    ...
    color = 0xED4245 if failed else (0x57F287 if filled >= total else get_embed_color_for("deploy_progress"))
...
def _deploy_success_embed(user: discord.Member, vps_count: int, container_name: str,
    ...
    embed = discord.Embed(
        title="🚀  Server Online",
        description=(
            f"**{user.mention}** — your VPS is live and ready to use.\n\n"
            f"```\n"
            f"  RAM   {ram} GB     CPU   {cpu} vCPU     DISK   {disk} GB\n"
            f"```"
        ),
        color=get_embed_color_for("deploy_success"),
        timestamp=_utcnow(),
    )
    if logo:
        embed.set_author(name=f"{get_brand_name()} VPS Hosting", icon_url=logo)
    ...
    embed.add_field(name="🔑  Access",    value="`!manage` → **SSH**",                      inline=True)
    embed.add_field(
        name="📩  Credentials",
        value=(
            "Login details & session links have been sent to your **DMs**.\n"
...
async def branding_cmd(interaction: discord.Interaction, name: str):
    ...
    try:
        ...
        pass
    # Push brand name + updated hostname to all running containers
    ...
    embed = discord.Embed(
        ...
        description=(
            f"All watermarks, embed footers, and **container hostnames** will now use **{name}**.\n\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• Container hostnames (applied live to running VPS)"
        ),
        inline=False,
    )
    if updated_containers:
        embed.add_field(
            name="🖥️  Live Updates",
            value=f"Hostname updated in **{updated_containers}** running container(s).",
            inline=True,
        )
...
# Preset palette shown as clickable buttons.  Each entry: (label, emoji, hex_int)
_COLOR_PALETTE: list[tuple[str, str, int]] = [
    ("Red",    "🔴", 0xE74C3C),
    ("Orange", "🟠", 0xE67E22),
    ("Gold",   "🟡", 0xF1C40F),
    ("Green",  "🟢", 0x2ECC71),
...
# Human-readable labels for each embed type key
_EMBED_TYPE_LABELS: dict[str, str] = {
    "deploy_progress": "🚀  Deploy Progress bar",
    "deploy_success":  "✅  Deploy Success embed",
    "dm":              "📩  DM embed (VPS credentials)",
    "general":         "🌐  General / all other embeds",
    "all":             "🎨  All embeds at once",
}

def _color_status_block() -> str:
    """Return a formatted block showing current colors for all embed types."""
    lines = []
    for key, label in _EMBED_TYPE_LABELS.items():
        if key == "all":
            continue
        c = get_embed_color_for(key)
        lines.append(f"{label}  →  `#{c:06X}`")
    return "\n".join(lines)

class _CustomHexModal(discord.ui.Modal, title="Enter Custom Hex Color"):
    hex_input = discord.ui.TextInput(
        label="Hex Color Code",
        placeholder="#1ABC9C  or  FF0000  (with or without #)",
        min_length=3,
        max_length=7,
        required=True,
    )

    def __init__(self, embed_type: str, caller_id: int):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if len(raw) != 6 or not all(c in "0123456789abcdefABCDEF" for c in raw):
            await interaction.response.send_message(
                embed=create_error_embed("Invalid Hex", f"`#{raw}` is not a valid 6-digit hex color."),
                ephemeral=True,
            )
            return
        color_int  = int(raw, 16)
        swatch     = f"#{raw.upper()}"
        _apply_embed_color(self.embed_type, color_int)
        label = _EMBED_TYPE_LABELS.get(self.embed_type, self.embed_type)
        logo  = get_logo_url()
        embed = discord.Embed(
            title="🎨  Color Updated",
            description=f"**{label}** is now `{swatch}`.",
            color=color_int,
            timestamp=_utcnow(),
        )
        embed.add_field(name="Current Colors", value=_color_status_block(), inline=False)
        kw: dict = {"text": f"{get_brand_name()}  •  /sidebarcolor"}
        if logo: kw["icon_url"] = logo
        embed.set_footer(**kw)
        await interaction.response.edit_message(embed=embed, view=None)

def _apply_embed_color(embed_type: str, color: int) -> None:
    """Apply a color to a single embed type or all at once."""
    if embed_type == "all":
        set_all_embed_colors(color)
    else:
        set_embed_color_for(embed_type, color)

class _ColorPaletteView(discord.ui.View):
    """Step 2: show 10 preset swatches + a 'Custom Hex' button."""
    ...
    def __init__(self, embed_type: str, caller_id: int):
        ...
        # Rows 0-1: 5 color buttons each
        for row_idx, (label, emoji, hex_int) in enumerate(_COLOR_PALETTE):
            btn = discord.ui.Button(
                label=label,
                emoji=emoji,
                style=discord.ButtonStyle.secondary,
                row=row_idx // 5,
                custom_id=f"color_{hex_int}",
            )
            # Capture loop variables via default argument
            async def make_callback(i: discord.Interaction, h=hex_int, l=label):
                if str(i.user.id) != str(self.caller_id):
                    await i.response.send_message("Not your panel.", ephemeral=True)
                    return
                _apply_embed_color(self.embed_type, h)
                swatch = f"#{h:06X}"
                type_label = _EMBED_TYPE_LABELS.get(self.embed_type, self.embed_type)
                logo = get_logo_url()
                emb = discord.Embed(
                    title="🎨  Color Updated",
                    description=f"**{type_label}** is now `{swatch}`.",
                    color=h,
                    timestamp=_utcnow(),
                )
                emb.add_field(name="Current Colors", value=_color_status_block(), inline=False)
                kw: dict = {"text": f"{get_brand_name()}  •  /sidebarcolor"}
                if logo: kw["icon_url"] = logo
                emb.set_footer(**kw)
        ...
        # Row 2: Custom Hex + Back
        custom_btn = discord.ui.Button(
            label="Custom Hex", emoji="✏️",
            style=discord.ButtonStyle.primary, row=2,
        )
        async def custom_callback(i: discord.Interaction):
            if str(i.user.id) != str(self.caller_id):
                await i.response.send_message("Not your panel.", ephemeral=True)
                return
            await i.response.send_modal(_CustomHexModal(self.embed_type, self.caller_id))
        ...
        async def back_callback(i: discord.Interaction):
            if str(i.user.id) != str(self.caller_id):
                ...
                return
            logo = get_logo_url()
            emb  = _sidebarcolor_home_embed()
            kw: dict = {"text": f"{get_brand_name()}  •  /sidebarcolor"}
            if logo: kw["icon_url"] = logo
            emb.set_footer(**kw)
            await i.response.edit_message(embed=emb, view=_EmbedTypeView(self.caller_id))
        back_btn.callback = back_callback
        self.add_item(back_btn)

    async def on_timeout(self):
        pass  # message stays; buttons just stop working

class _EmbedTypeView(discord.ui.View):
    """Step 1: select which embed type to color."""
    ...
    def __init__(self, caller_id: int):
        ...
        options = [
            discord.SelectOption(
                ...
                description=_embed_type_hint(key),
                value=key,
                emoji=label.split("  ")[0],
            )
            for key, label in _EMBED_TYPE_LABELS.items()
        ...
        sel = discord.ui.Select(
            placeholder="🎨  Which embed do you want to color?",
            options=options,
            min_values=1,
            max_values=1,
            row=0,
        )
        async def sel_callback(i: discord.Interaction):
            if str(i.user.id) != str(self.caller_id):
                await i.response.send_message("Not your panel.", ephemeral=True)
                return
            embed_type = sel.values[0]
            label      = _EMBED_TYPE_LABELS[embed_type]
            logo       = get_logo_url()
            cur_color  = get_embed_color_for(embed_type) if embed_type != "all" else get_embed_color()
            emb = discord.Embed(
                title=f"🎨  Pick a color for {label}",
                description=f"Current color: `#{cur_color:06X}`\n\nChoose a preset or enter a custom hex code.",
                color=cur_color,
                timestamp=_utcnow(),
            )
            kw: dict = {"text": f"{get_brand_name()}  •  /sidebarcolor"}
            if logo: kw["icon_url"] = logo
            emb.set_footer(**kw)
            await i.response.edit_message(embed=emb, view=_ColorPaletteView(embed_type, self.caller_id))
        sel.callback = sel_callback
        self.add_item(sel)

def _embed_type_hint(key: str) -> str:
    hints = {
        "deploy_progress": "In-progress deploy bar color",
        "deploy_success":  "Server Online channel embed",
        "dm":              "Your VPS is Live! DM card",
        "general":         "Fallback for all other bot embeds",
        "all":             "Apply one color to every embed at once",
    }
    return hints.get(key, "")

def _sidebarcolor_home_embed() -> discord.Embed:
    return discord.Embed(
        title="🎨  Sidebar Color Manager",
        description=(
            "Select which embed you want to customize from the dropdown below.\n\n"
            + _color_status_block()
        ),
        color=get_embed_color_for("general"),
        timestamp=_utcnow(),
    )

@bot.tree.command(name="sidebarcolor", description="Customize sidebar colors for each embed type (Admin only)")
async def sidebarcolor_cmd(interaction: discord.Interaction):
    """Admin-only: interactive color picker for each embed type."""
    user_id = str(interaction.user.id)
    if user_id != str(MAIN_ADMIN_ID) and user_id not in admin_data.get("admins", []):
        await interaction.response.send_message(
            embed=create_error_embed("Permission Denied", "Only admins can change embed colors."),
            ephemeral=True,
        )
        return

    logo  = get_logo_url()
    embed = _sidebarcolor_home_embed()
    kw: dict = {"text": f"{get_brand_name()}  •  /sidebarcolor"}
    if logo: kw["icon_url"] = logo
    embed.set_footer(**kw)
    await interaction.response.send_message(embed=embed, view=_EmbedTypeView(interaction.user.id), ephemeral=True)
