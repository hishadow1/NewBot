...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    ALL of the following checks are critical — deployment fails if any one fails:
      ✓ PID 1 is systemd
      ✓ systemctl is responsive
      ✓ journalctl is responsive
    ...
    # ── Give sshd one chance to come up if it isn't yet ───────────────────────
    # (systemd may still be activating services right after configure_vps)
    ...
    journal_ok  = await run_check("journalctl",         "journalctl --no-pager -n 1 2>&1",   "")
    ssh_ok      = await run_check("SSH daemon",         "pgrep -x sshd && echo sshd_running", "sshd_running")

    # Docker checks — give the socket one retry if the first attempt fails.
    # Use --format to avoid relying on human-readable table headers (e.g. "REPOSITORY",
    # "CONTAINER") which vary between Docker versions.  A zero exit-code is sufficient
    # to confirm the CLI can reach the host daemon.
    ...
    docker_ps_ok   = await run_check("docker ps",
                                     ...
                                     None)
    docker_img_ok  = await run_check("docker images",
                                     "docker images --format '{{.Repository}}' 2>&1",
                                     None)

    if not (docker_ver_ok and docker_ps_ok and docker_img_ok):
        # Socket permission may still be propagating — wait and retry once
        await asyncio.sleep(3)
        ...
        if not docker_img_ok:
            docker_img_ok  = await run_check("docker images (retry)",
                                             "docker images --format '{{.Repository}}' 2>&1",
                                             None)

    hostname_ok = await run_check("Hostname",           "hostname",              hostname or VPS_HOSTNAME)

    # SSH retry if it missed the initial window
    if not ssh_ok:
        await asyncio.sleep(5)
        ssh_ok = await run_check("SSH daemon (retry)",  "pgrep -x sshd && echo sshd_running", "sshd_running")

    # ── Build human-readable report ───────────────────────────────────────────
    # Critical checks: deployment fails only if one of these is False.
    # Docker checks are optional — a warning is logged but VPS deployment succeeds.
    _CRITICAL = frozenset({
        "PID 1 is systemd", "systemctl", "journalctl",
        "SSH daemon", "SSH daemon (retry)", "Hostname",
    })
    _DOCKER = frozenset({
        "docker version", "docker ps", "docker images",
        "docker version (retry)", "docker ps (retry)", "docker images (retry)",
    })
    ...
    if all_passed:
        if docker_warnings:
            logger.warning(
                f"VPS {container_name} verification PASSED (Docker optional checks "
                f"failed: {docker_warnings}).\n{report}"
            )
        else:
...
while [ $_waited -lt 20 ]; do
    _url=$(grep -o 'https://sshx\.io/s/[^[:space:]]*' /tmp/sshx.log 2>/dev/null | head -1)
    if [ -n "$_url" ]; then
        echo "SSHX_URL:${_url}"
        exit 0
    fi
...
echo "SSHX_URL:"
"""
    stdout, stderr, rc = await docker_exec(container_name, start_script, timeout=30)

    for line in stdout.splitlines():
        if line.startswith("SSHX_URL:"):
            url = line[len("SSHX_URL:"):].strip()
            if url:
                return url

    raise Exception(f"sshx URL not found in output. stderr: {stderr[:300]}")
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                try:
                    ...
                    vps["tmate_web"] = tmate_info.get("web", "")
                    save_data()
                ...
                ssh_embed = create_embed(
                    "🔑 VPS Access Methods",
                    f"All available connection methods for `{container_name}`:",
                    0x00ff88
                )

                # 1. OpenSSH
                if host_ip and stored_port:
                    ssh_embed.add_field(
                        name="🔐 SSH  (OpenSSH)",
                        value=f"```ssh root@{host_ip} -p {stored_port}```\nPassword: `{ssh_password}`",
                        inline=False
                    )
                else:
                    ssh_embed.add_field(
                        name="🔐 SSH  (OpenSSH)",
                        value=f"Password: `{ssh_password}`\n_(host IP not configured — set SERVER_IP env var)_",
                        inline=False
                    )
                ...
                if tmate_info.get("ssh"):
                    ...
                    ssh_embed.add_field(name="🖥️ tmate SSH", value=f"⚠️ Failed: `{tmate_err[:120]}`", inline=False)

                # 3. tmate Web
                if tmate_info.get("web"):
                    ssh_embed.add_field(name="🌐 tmate Web", value=tmate_info["web"], inline=False)

                # 4. sshx
                if sshx_url:
                    ssh_embed.add_field(name="🔗 sshx Web", value=sshx_url, inline=False)
                elif sshx_err:
                    ssh_embed.add_field(name="🔗 sshx Web", value=f"⚠️ Failed: `{sshx_err[:120]}`", inline=False)

                ssh_embed.add_field(
                    name="📌 Notes",
                    value=(
                        "• OpenSSH is always available while the VPS is running\n"
                        "• tmate/sshx links refresh each time you click SSH\n"
                ...
                try:
                    ...
                    await interaction.followup.send(
                        embed=create_success_embed("Access Details Sent", "Check your DMs for all connection methods!"),
                        ephemeral=True)
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        ...
        try:
            ...
            vps_info["tmate_web"] = tmate_info.get("web", "")
            save_data()
        except Exception as _te:
        ...
        try:
            host_ip = await _get_server_ip()
            dm_embed = create_embed("🎉 Your VPS is Ready!", "Here are all your access methods:", color=0x5865F2)
            ...
            if host_ip and ssh_port:
                dm_embed.add_field(
                    name="🔐 SSH  (OpenSSH)",
                    value=f"```ssh root@{host_ip} -p {ssh_port}```\nPassword: `{password}`",
                    inline=False
                )
            if tmate_info.get("ssh"):
                dm_embed.add_field(name="🖥️ tmate SSH", value=f"```{tmate_info['ssh']}```", inline=False)
            if tmate_info.get("web"):
                dm_embed.add_field(name="🌐 tmate Web", value=tmate_info["web"], inline=False)
            if sshx_url:
                dm_embed.add_field(name="🔗 sshx Web", value=sshx_url, inline=False)
            dm_embed.add_field(
                name="📌 Notes",
                value=(
                    "• OpenSSH stays alive as long as your VPS is running\n"
                    "• tmate/sshx sessions refresh each time you click SSH in `!manage`\n"
                    "• Use `!manage` to start, stop, reinstall or get fresh session links"
                ),
                inline=False
            )
            dm_embed.set_footer(text="DarkNodes VPS  •  Keep this message safe")
