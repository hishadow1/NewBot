...
def create_embed(title, description="", color=0x000000, fields=None):
    logo  = get_logo_url()
    ...
    kw: dict = {"text": f"DarkNodes | VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
def _deploy_progress_embed(step_index: int, steps: list = None, failed: bool = False) -> discord.Embed:
    ...
    color = 0xff4444 if failed else (0x00ff88 if filled >= total else 0x000000)
    ...
    title = "❌  DarkNodes VPS — Deploy Failed" if failed else "DarkNodes VPS — Deploying 🚀"
    logo  = get_logo_url()
    embed = discord.Embed(
        title=title,
        description=f"**`{bar}`**\n**{pct}%**\n\n" + "\n".join(lines),
...
def _vps_dm_embed(vps_count: int, container_name: str, ram: str, cpu: str,
    ...
    logo     = get_logo_url()
    embed = discord.Embed(
        title="🎉  DarkNodes — Your VPS is Ready!",
        description=(
    ...
    if tmate_ssh or sshx_url:
        ...
        if sshx_url:
            access_parts.append(f"**🔗 sshx Web**\n{sshx_url}")
    ...
    kw: dict = {"text": "DarkNodes VPS  •  Keep this DM safe"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    docker_svc_ok = await run_check(
        ...
        # No "|| echo inactive" fallback — when the service is not active
        # systemctl exits non-zero and run_check correctly fails on rc != 0.
        # Without the fallback "inactive" would falsely match "active" as a substring.
    ...
    docker_info_ok = await run_check(
        "Docker daemon info",
        # set -o pipefail makes the pipeline exit with docker info's exit code,
        # not head's exit code — without it a failing docker info looks like rc=0.
        "bash -c 'set -o pipefail; docker info 2>&1 | head -20'",
        "Server Version",
        timeout=30,
    )
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        _log.set_author(name="VPS Reinstalled  •  Fresh OS", icon_url=get_logo_url() or discord.embeds.EmptyEmbed)
                        try:
            ...
            try:
                ...
                _log.set_author(name="VPS Started  •  Container Online", icon_url=get_logo_url() or discord.embeds.EmptyEmbed)
                try:
                    _log.set_thumbnail(url=interaction.user.display_avatar.url)
                except Exception:
                    pass
                ...
                _log.add_field(name="🌐 Status",     value="`🟢 RUNNING`",        inline=True)
                _sl = get_logo_url()
                _log.set_footer(**{"text": "DarkNodes VPS Logs  •  Start Event", **({"icon_url": _sl} if _sl else {})})
                asyncio.create_task(send_log(_log))
            except Exception as e:
                await interaction.followup.send(
