...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None):
    """
...
# ── 1b. Mask rescue/emergency units inside Docker ─────────────────────────────
...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    Critical checks (deployment fails if any fail):
      ✓ PID 1 is systemd
      ✓ systemctl --version
      ✓ systemctl is-active ssh
      ✓ Hostname matches expected value

    Optional checks (warning only, deployment still succeeds):
      ✓ docker version / docker ps
      ✓ docker run hello-world
      ✓ tmate installed
      ✓ sshx installed

    Note: "maintenance" from systemctl is-system-running is normal inside Docker
    when rescue/emergency units fail — does NOT fail deployment.
    """
    ...
    # ── Give systemd a moment to activate services ────────────────────────────
    ...
    ssh_ok       = await run_check("SSH service",       "systemctl is-active ssh 2>&1", "active")

    if not ssh_ok:
        await asyncio.sleep(5)
        ssh_ok = await run_check("SSH service (retry)", "systemctl is-active ssh 2>&1", "active")

    hostname_ok  = await run_check("Hostname",          "hostname",            hostname or VPS_HOSTNAME)

    # ── Optional: Docker checks ───────────────────────────────────────────────
    docker_ver_ok = await run_check("docker version",
                                    "docker version 2>&1",
                                    "Server")
    docker_ps_ok  = await run_check("docker ps",
                                    "docker ps --format '{{.ID}}' 2>&1",
                                    None)

    if not (docker_ver_ok and docker_ps_ok):
        await asyncio.sleep(3)
        if not docker_ver_ok:
            docker_ver_ok = await run_check("docker version (retry)",
                                            "docker version 2>&1",
                                            "Server")
        if not docker_ps_ok:
            docker_ps_ok  = await run_check("docker ps (retry)",
                                            "docker ps --format '{{.ID}}' 2>&1",
                                            None)

    docker_run_ok = await run_check("docker run hello-world",
                                    "docker run --rm hello-world 2>&1 | tail -5",
                                    "Hello from Docker",
                                    timeout=45)

    # ── Optional: tool checks ─────────────────────────────────────────────────
    tmate_ok = await run_check("tmate installed", "command -v tmate && tmate -V 2>&1", "tmate")
    sshx_ok  = await run_check("sshx installed",  "command -v sshx 2>&1",              "sshx")

    # ── Build human-readable report ───────────────────────────────────────────
    _CRITICAL = frozenset({
        "PID 1 is systemd", "systemctl",
        "SSH service", "SSH service (retry)", "Hostname",
    })
    _OPTIONAL = frozenset({
        "docker version", "docker ps", "docker run hello-world",
        "docker version (retry)", "docker ps (retry)",
    ...
    critical_failed = [
        lbl for lbl, (ok, _) in results.items()
        if lbl in _CRITICAL and not ok
    ]
    optional_warnings = [
        lbl for lbl, (ok, _) in results.items()
        if lbl in _OPTIONAL and not ok
    ]
    ...
    if all_passed:
        if optional_warnings:
            logger.warning(
                f"VPS {container_name} verification PASSED (optional checks "
                f"failed: {optional_warnings}).\n{report}"
            )
        else:
            logger.info(f"VPS {container_name} verification PASSED.\n{report}")
    else:
        logger.error(
            f"VPS {container_name} verification FAILED — critical: {critical_failed}.\n{report}"
        )
...
async def get_sshx_session(container_name: str) -> str:
    """
    Start an sshx session inside the container and return the web URL.

    sshx is pre-installed in the image at /usr/local/bin/sshx.
    Raises Exception on failure so callers can treat it as optional.
    """
    chk_out, _, chk_rc = await docker_exec(
    ...
    # Use in-shell sed to strip ANSI before extracting the URL.
    start_script = r"""
pkill sshx 2>/dev/null || true
sleep 1
sshx > /tmp/sshx.raw 2>&1 &
_waited=0
while [ $_waited -lt 25 ]; do
    _url=$(sed 's/\x1b\[[0-9;]*[a-zA-Z]//g' /tmp/sshx.raw 2>/dev/null \
           | grep -o 'https://sshx\.io/s/[A-Za-z0-9_/#-]*' \
           | head -1)
    if [ -n "$_url" ]; then
        echo "SSHX_URL:${_url}"
        exit 0
...
echo "SSHX_URL:"
sed 's/\x1b\[[0-9;]*[a-zA-Z]//g' /tmp/sshx.raw 2>/dev/null | head -10
"""
    stdout, stderr, rc = await docker_exec(container_name, start_script, timeout=35)

    # Strip any ANSI that leaked through docker exec itself
    ansi_re = re.compile(r'\x1b\[[0-9;]*[a-zA-Z]')

    for line in stdout.splitlines():
        if line.startswith("SSHX_URL:"):
            url = ansi_re.sub("", line[len("SSHX_URL:"):]).strip()
            if url and re.match(r'^https://sshx\.io/s/[A-Za-z0-9_/#-]+$', url):
                return url
            elif url:
                raise Exception(f"sshx returned invalid URL: {repr(url)}")

    log_lines = [l for l in stdout.splitlines() if not l.startswith("SSHX_URL:")]
    log_ctx = "\n".join(log_lines[:10]).strip()
    raise Exception(
        f"sshx URL not found.\nLog: {log_ctx[:300] or '(empty)'}\nstderr: {stderr[:200]}"
    )
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                ssh_embed = discord.Embed(
                    title="🔗 DarkNodes Access",
                    description=(
                        f"━━━━━━━━━━━━━━━━━━\n"
                        f"Connection methods for `{container_name}`"
                    ),
                    color=0x5865F2
                )
                ...
                if tmate_info.get("ssh"):
                    ...
                    ssh_embed.add_field(
                        name="🖥️ tmate SSH",
                        value=f"⚠️ Unavailable: `{tmate_err[:120]}`",
                        inline=False
                    )
                ...
                if sshx_url:
                    ...
                    ssh_embed.add_field(
                        name="🌐 sshx",
                        value=f"⚠️ Unavailable: `{sshx_err[:120]}`",
                        inline=False
                    )

                ssh_embed.add_field(
                    name="📌 Note",
                    value="Links are refreshed each time you click the SSH button.",
                    inline=False
                )
                ssh_embed.set_footer(text="DarkNodes VPS Platform")

                try:
                    await interaction.user.send(embed=ssh_embed)
                    await interaction.followup.send(
                        embed=create_success_embed("Access Details Sent", "Check your DMs for connection details!"),
                        ephemeral=True)
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    def _make_deploy_progress(step: str) -> discord.Embed:
        e = discord.Embed(
            title="🚀 DarkNodes VPS — Deploying",
            description=f"━━━━━━━━━━━━━━━━━━\n⏳ {step}",
            color=0x00ccff
        )
        ...
        e.add_field(name="🏷️ Hostname", value=f"`{vps_hostname}`",  inline=True)
        e.set_footer(text="DarkNodes VPS Platform")
        return e

    progress_msg = await ctx.send(embed=_make_deploy_progress("Creating container…"))

    ssh_port = get_next_ssh_port()
    try:
        await progress_msg.edit(embed=_make_deploy_progress("Booting systemd…"))
        await create_docker_container(container_name, ram_mb, cpu, ssh_port, password, disk_gb=disk, hostname=vps_hostname)
        ...
        # ── Optional: tmate ──────────────────────────────────────────────────────
        await progress_msg.edit(embed=_make_deploy_progress("Creating access methods…"))
        tmate_info = {}
        try:
            tmate_info = await get_tmate_session(container_name)
        ...
        # ── Build final deployment embed ─────────────────────────────────
        _now = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
        deploy_embed = discord.Embed(
            title="🚀 DarkNodes VPS Created",
            description=(
                f"━━━━━━━━━━━━━━━━━━\n"
                f"{user.mention} your VPS is live! Check your **DMs** for access details."
            ),
            color=0x00ff88
        )
        deploy_embed.add_field(name="📦 VPS", value=(
            f"**Name:** `{container_name}`\n"
            f"**Hostname:** `{vps_hostname}`\n"
            f"**VPS ID:** `#{vps_count}`"
        ), inline=True)
        deploy_embed.add_field(name="💻 Resources", value=(
            f"**RAM:** `{ram} GB`\n"
            f"**CPU:** `{cpu} Core(s)`\n"
            f"**Disk:** `{disk} GB`"
        ), inline=True)
        _t_ok = "✅ Ready" if tmate_info.get("ssh") else "⚠️ Unavailable"
        _s_ok = "✅ Ready" if sshx_url              else "⚠️ Unavailable"
        deploy_embed.add_field(name="🟢 Status", value=(
            f"**Systemd:** ✅ Running\n"
            f"**Docker:** ✅ Active\n"
            f"**tmate:** {_t_ok}\n"
            f"**sshx:** {_s_ok}"
        ...
        _access = ""
        if tmate_info.get("ssh"):
            _access += f"**tmate SSH:** `{tmate_info['ssh'][:60]}…`\n"
        if sshx_url:
            _access += f"**sshx:** {sshx_url}"
        deploy_embed.add_field(
            name="🔗 Access",
            value=_access.strip() or "_Use `!manage` → SSH to generate links_",
            inline=False
        )
        deploy_embed.add_field(name="⏱ Deployment", value=(
            f"**Owner:** {user.mention}\n"
            f"**Deployed By:** {ctx.author.mention}\n"
            f"**Completed:** `{_now}`\n"
            f"**Manage:** `!manage`"
        ), inline=False)
        deploy_embed.set_footer(text="DarkNodes VPS Platform")
        await progress_msg.edit(embed=deploy_embed)

        # ── DM ───────────────────────────────────────────────────────────
        try:
            dm_embed = discord.Embed(
                title="🎉 Your DarkNodes VPS is Ready!",
                description="━━━━━━━━━━━━━━━━━━\nHere are your access details.",
                color=0x5865F2
            )
            dm_embed.add_field(name="📦 VPS",  value=f"`{container_name}`", inline=True)
            dm_embed.add_field(name="🧠 RAM",  value=f"`{ram} GB`",         inline=True)
            dm_embed.add_field(name="⚙️ CPU",  value=f"`{cpu} Core(s)`",    inline=True)
            if tmate_info.get("ssh"):
                dm_embed.add_field(name="🖥️ tmate SSH", value=f"```{tmate_info['ssh']}```", inline=False)
            if sshx_url:
                dm_embed.add_field(name="🌐 sshx", value=sshx_url, inline=False)
            dm_embed.add_field(
                name="📌 Note",
                value=(
                    "• tmate/sshx links refresh each time you click SSH in `!manage`\n"
                    "• Use `!manage` to start, stop, or reinstall your VPS"
                ),
                inline=False
            )
            dm_embed.set_footer(text="DarkNodes VPS Platform  •  Keep this safe")
        ...
        err_embed = create_error_embed("Deployment Failed", f"Error: {str(e)}")
        try:
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    def _make_buywc_progress(step: str) -> discord.Embed:
        e = discord.Embed(
            title="🚀 DarkNodes VPS — Deploying",
            description=f"━━━━━━━━━━━━━━━━━━\n⏳ {step}",
            color=0x00ccff
        ...
        e.add_field(name="🧠 RAM",      value=f"`{ram_str}`",             inline=True)
        e.add_field(name="⚙️ CPU",      value=f"`{cpu_str} Core(s)`",     inline=True)
        e.add_field(name="🏷️ Hostname", value=f"`{vps_hostname}`",        inline=True)
        e.set_footer(text="DarkNodes VPS Platform")
        return e

    progress_msg = await ctx.send(embed=_make_buywc_progress("Creating container…"))

    ssh_port = get_next_ssh_port()
    try:
        await progress_msg.edit(embed=_make_buywc_progress("Booting systemd…"))
        await create_docker_container(container_name, ram_mb, cpu_str, ssh_port, password, hostname=vps_hostname)
        ...
        # ── Optional: tmate ──────────────────────────────────────────────────────
        await progress_msg.edit(embed=_make_buywc_progress("Creating access methods…"))
        tmate_info = {}
        try:
            tmate_info = await get_tmate_session(container_name)
            vps_info["tmate_ssh"] = tmate_info.get("ssh", "")
        ...
        deploy_embed = discord.Embed(
            ...
            description=(
                f"━━━━━━━━━━━━━━━━━━\n"
                f"{ctx.author.mention} your VPS is live! Check your **DMs** for access details."
            ),
            color=0x00ff88
        )
        deploy_embed.add_field(name="📦 VPS", value=(
        ...
        deploy_embed.add_field(name="💻 Resources", value=(
            f"**RAM:** `{ram_str}`\n"
            f"**CPU:** `{cpu_str} Core(s)`\n"
            f"**Plan:** `{plan}` ({processor})"
        ), inline=True)
        _t_ok = "✅ Ready" if tmate_info.get("ssh") else "⚠️ Unavailable"
        _s_ok = "✅ Ready" if sshx_url              else "⚠️ Unavailable"
        deploy_embed.add_field(name="🟢 Status", value=(
        ...
        try:
            ...
            dm_embed.add_field(name="📋 Plan",  value=f"`{plan}` ({processor})", inline=True)
        ...
        try:
            ...
            pass
        err_embed = create_error_embed("Purchase Failed", f"Error: {str(e)}\n\nCredits refunded.")
        try:
            await progress_msg.edit(embed=err_embed)
        except Exception:
            await ctx.send(embed=err_embed)
...
# ─── Template System ──────────────────────────────────────────────────────────
# Single unified module — registers /template with Pterodactyl + Cloudflare Tunnel built in.
import template
