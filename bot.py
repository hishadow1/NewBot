...
def _deploy_success_embed(user: discord.Member, vps_count: int, container_name: str,
    ...
    embed = discord.Embed(
        ...
        description=(
            f"**`{bar}`**  **100%**\n\n"
            + "\n".join(lines)
            + "\n\n> Your VPS is **live** and fully configured."
        ),
        color=0x57F287,
        timestamp=_utcnow(),
    )
    ...
    embed.add_field(
        name="🔑 Get Access",
        value="Use `!manage` → **SSH** to generate live **tmate** & **sshx** session links.",
        inline=False,
    )
...
async def dind_autoheal(container_name: str, delay: int = 180, notify_user: discord.Member = None):
    ...
    logger.warning(f"[autoheal] DinD in {container_name} not ready after {delay}s — restarting docker service.")

    # ── Step 2: restart docker inside the container ───────────────────────────
    try:
        await docker_exec(container_name, "systemctl restart docker 2>&1", timeout=20)
    except Exception as e:
        logger.warning(f"[autoheal] systemctl restart docker failed: {e}")
