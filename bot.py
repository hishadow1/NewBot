...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    # Poll until dockerd socket actually responds (up to 45 s).
    ...
    docker_info_ok = await run_check(
        ...
        (
            r"bash -c '"
            r"for i in $(seq 1 45); do "
            r"  out=$(docker info 2>&1); "
            r"  if echo \"$out\" | grep -q \"Server Version\"; then "
            r"    echo \"$out\" | head -20; exit 0; "
            r"  fi; "
            r"  sleep 1; "
            r"done; "
            r"echo \"Timed out — dockerd socket not ready after 45s\"; exit 1'"
        ),
        "Server Version",
        timeout=60,
    )
