...
def _deploy_success_embed(user: discord.Member, vps_count: int, container_name: str,
    ...
    kw: dict = {"text": f"{get_brand_name()} VPS  •  Deployed & Ready"}
    if logo:
        kw["icon_url"] = logo
    embed.set_footer(**kw)
    return embed
...
echo "  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚══════╝"
echo ""
echo "  Welcome to your {get_brand_name()} VPS"
echo "  ─────────────────────────────────────────────────────────────"
printf "  %-16s %s\n" "Hostname:"   "$_hn"
printf "  %-16s %s\n" "OS:"         "$_os"
printf "  %-16s %s\n" "Kernel:"     "$_kern"
...
async def branding_cmd(interaction: discord.Interaction, name: str):
    ...
    set_brand_name(name)
    # Also keep the template system in sync (it reads via the injected getter,
    # so no extra call is needed — but re-syncing activity confirms the change)
    try:
        await bot.change_presence(activity=discord.Activity(
            type=discord.ActivityType.watching, name=f"{get_brand_name()} | VPS Manager"
        ))
