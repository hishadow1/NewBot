...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None):
    """
    Create a Docker container that behaves as closely as possible to a real VPS.

    Design notes:
    - --privileged is required to let systemd function as PID 1 inside the
      container (cgroup management, service units, journald, etc.).
      Trade-off: privileged containers have elevated host access. This is an
      acceptable risk for a VPS-hosting bot where each container is a
      deliberate user allocation.
    - --cgroupns=host exposes the real cgroup hierarchy (required for
      cgroup-v2 hosts running Ubuntu 22+).
    - tmpfs mounts for /run and /tmp allow systemd to write its runtime state
      without needing a writable host directory.
    - We attempt to boot with /sbin/init (→ systemd on Ubuntu 22.04).  If the
      image has no working init binary the fallback is 'sleep infinity', which
      keeps the container alive so packages can still be installed; systemctl
      commands will not function in that mode but SSH and basic tooling will.
    - Disk quota via --storage-opt is only honoured when the Docker daemon uses
      the overlay2 driver with project quotas enabled. If the daemon does not
      support it the flag is silently omitted (see try/except below).
    """

    # 1. Pull image if not already cached (silent failure if offline)
    try:
    ...
    #    We try /sbin/init first; if that fails we recreate with sleep infinity.
    ...
    run_cmd_init     = f"docker run -d {base_flags}{DOCKER_IMAGE} /sbin/init"
    run_cmd_fallback = f"docker run -d {base_flags}{DOCKER_IMAGE} sleep infinity"

    started_with_init = True
    try:
        await execute_docker(run_cmd_init, timeout=60)
        logger.info(f"Container {container_name} started with /sbin/init")
    except Exception as init_err:
        logger.warning(
            f"Container {container_name}: /sbin/init failed ({init_err}), "
            "falling back to sleep infinity. systemctl will not be available."
        )
        started_with_init = False
        try:
            # Clean up any partial container before retrying
            await execute_docker(f"docker rm -f {container_name}", timeout=15)
        except Exception:
            pass
        await execute_docker(run_cmd_fallback, timeout=60)

    # 3. Wait for the container to settle before running exec commands
    await asyncio.sleep(3)

    # 4. Full VPS initialisation (installs packages, configures SSH etc.)
    await initialize_vps(container_name, password, started_with_init, hostname=_hostname)

    return True


async def initialize_vps(container_name: str, password: str, has_init: bool = True, hostname: str = None):
    """
    Install and configure everything needed for a real VPS experience.
    Runs inside the container after it has started.
    Safe to call again on reinstall.
    """
    _hostname = hostname or VPS_HOSTNAME
    logger.info(f"Initialising VPS {container_name} (hostname={_hostname}) …")

    # ── Package installation ──────────────────────────────────────────────────
    # We split into two apt-get calls: first the init layer (dbus, systemd),
    # then the user-facing tools.  This way a partial failure on the optional
    # tools still leaves SSH working.
    ...
    tool_packages = (
        "curl wget nano vim git python3 python3-pip "
        "unzip zip docker.io iproute2 net-tools procps htop"
    )
...
# ── Core packages (SSH, systemd, dbus) ──────────────────────────────────────
apt-get install -y --no-install-recommends {core_packages} -qq

# ── User tools ───────────────────────────────────────────────────────────────
apt-get install -y --no-install-recommends {tool_packages} -qq || true
...
# Ensure desired SSH options are set (append if not already present)
...
# Disable PAM (can cause login hangs inside containers)
...
if command -v systemctl >/dev/null 2>&1 && systemctl is-system-running >/dev/null 2>&1; then
    ...
    systemctl restart ssh 2>/dev/null || /usr/sbin/sshd -D &
else
    # No systemd PID1 – start sshd directly
    pkill sshd 2>/dev/null || true
    /usr/sbin/sshd
fi

# ── Docker group ─────────────────────────────────────────────────────────────
# NOTE: Docker-in-Docker is not fully supported without a separate dockerd.
# docker.io is installed so the CLI and libraries are available but the daemon
# cannot start inside a privileged container on all hosts.
...
echo "DARKNODES_INIT_COMPLETE"
"""

    try:
        stdout, stderr, rc = await docker_exec(container_name, setup_script, timeout=300)
        if "DARKNODES_INIT_COMPLETE" not in stdout:
            logger.warning(
                f"VPS {container_name} init script did not confirm completion. "
                f"rc={rc}  stderr={stderr[-300:] if stderr else ''}"
            )
        else:
            logger.info(f"VPS {container_name} initialisation complete.")
    except Exception as e:
        logger.error(f"VPS {container_name} init error: {e}")
        # Do NOT re-raise — we don't want a partially successful setup to kill
        # the whole deployment. Errors are shown in the verification step.

    # Run verification and log results (non-fatal)
    await verify_vps(container_name, hostname=_hostname)


async def verify_vps(container_name: str, hostname: str = None) -> Dict[str, bool]:
    """
    Verify that key VPS features are working after initialisation.
    Returns a dict of check → bool.
    Logs warnings for any failed checks but never raises.
    """
    checks = {
        "hostname":   False,
        "systemctl":  False,
    ...
    async def run_check(cmd: str, expect: str = None) -> bool:
        try:
            stdout, _, rc = await docker_exec(container_name, cmd, timeout=15)
            if rc != 0:
                return False
            if expect and expect.lower() not in stdout.lower():
                return False
            return True
        except Exception:
            return False

    checks["hostname"]   = await run_check("hostname",                     hostname or VPS_HOSTNAME)
    checks["systemctl"]  = await run_check("systemctl --version 2>&1",     "systemd")
    checks["journalctl"] = await run_check("journalctl --version 2>&1",    "systemd")
    checks["service"]    = await run_check("service --status-all 2>&1 || true", None)
    checks["ssh"]        = await run_check("pgrep -x sshd && echo ok",     "ok")
    checks["docker_cli"] = await run_check("docker --version 2>&1",        "Docker")

    failed = [k for k, v in checks.items() if not v]
    if failed:
        logger.warning(f"VPS {container_name} verification: failed checks = {failed}")

        # Retry: if SSH is down, try to start it again
        if "ssh" in failed:
            try:
                await docker_exec(
                    container_name,
                    "pkill sshd 2>/dev/null || true; sleep 1; /usr/sbin/sshd",
                    timeout=15
                )
                checks["ssh"] = await run_check("pgrep -x sshd && echo ok", "ok")
                if checks["ssh"]:
                    logger.info(f"VPS {container_name}: SSH recovered on retry.")
            except Exception:
                pass
    else:
        logger.info(f"VPS {container_name}: all verification checks passed.")

    return checks


async def get_tmate_session(container_name):
    """Start tmate inside the container and return the SSH command string."""
    tmate_script = (
    ...
    # Ensure tmate is installed (it may not be in the base packages list)
    install_check = (
        "command -v tmate >/dev/null 2>&1 || "
        "apt-get install -y tmate -qq 2>/dev/null || true"
    )
    await docker_exec(container_name, install_check, timeout=60)
...
async def _suspend_vps_for_mining(
    ...
    if channel_id:
        try:
            ...
            logger.error(f"Failed to send mining alert to channel: {notify_err}")
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                ssh_embed.add_field(
                    name="⚠️ Note",
                    value=(
                        "• tmate session expires when VPS restarts\n"
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    await ctx.send(embed=create_info_embed(
        "Creating VPS",
        f"Deploying Docker VPS for {user.mention}…\n"
        f"🧠 RAM: `{ram}GB` | ⚙️ CPU: `{cpu} Core(s)` | 💾 Disk: `{disk}GB`\n"
        f"⏳ This may take 2-3 minutes while packages are installed."
    ))

    try:
        await create_docker_container(container_name, ram_mb, cpu, 0, password, disk_gb=disk, hostname=vps_hostname)
        ...
        try:
            ...
            dm_embed.add_field(name="🔗 SSH Command",       value=f"```{tmate_cmd}```", inline=False)
            dm_embed.add_field(name="📌 How to Connect",
                               value="1️⃣ Copy the command above\n2️⃣ Paste in CMD / Termux / PuTTY\n3️⃣ You\'re in! 🚀",
                               inline=False)
            dm_embed.add_field(name="🎮 Manage Your VPS",
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    await ctx.send(embed=create_info_embed(
        "Processing Purchase",
        f"Deploying {plan} Docker VPS…\n⏳ Please wait 2-3 minutes for full setup."
    ))

    try:
        await create_docker_container(container_name, ram_mb, cpu_str, 0, password, hostname=vps_hostname)
...
@bot.tree.command(name="setlogchannel", description="Set the channel where mining/suspension alerts are posted (Admin only)")
@discord.app_commands.describe(channel="The text channel that will receive suspension alert embeds")
async def setlogchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    """Admin-only slash command: choose the log channel for anti-mining alerts."""
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Mining & suspension alerts will now be sent to {channel.mention}.\n\n"
            f"**Channel:** `#{channel.name}` (ID `{channel.id}`)\n"
            f"Whenever the anti-abuse monitor auto-suspends a VPS, a detailed alert embed "
            f"will appear in that channel instantly."
        ),
        colour=discord.Colour.green(),
        timestamp=datetime.utcnow()
    )
    embed.set_footer(text="DarkNodes Anti-Abuse System  •  /setlogchannel")
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="🚨 Anti-Mining & Abuse", value=(
            ...
            "`/setlogchannel #channel` — Set the alert log channel (slash command)"
        ), inline=False)
