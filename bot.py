...
async def dind_autoheal(container_name: str, delay: int = 180):
    """
    ...
    Attempt sequence:
      1. Try `docker info` — if it works, we're done.
    ...
    # ── Step 1: check ─────────────────────────────────────────────────────────
    if await _dind_ok():
        # Already healthy — silent, no log spam
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        ...
        asyncio.create_task(dind_autoheal(container_name, delay=180))
