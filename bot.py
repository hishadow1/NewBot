...
async def verify_vps(container_name: str, hostname: str = None) -> tuple:
    ...
    docker_ps_ok   = await run_check("docker ps",       "docker ps 2>&1",       "CONTAINER")
    docker_img_ok  = await run_check("docker images",   "docker images 2>&1",   "REPOSITORY")
    ...
    if not (docker_ver_ok and docker_ps_ok and docker_img_ok):
        ...
        if not docker_ps_ok:
            docker_ps_ok   = await run_check("docker ps (retry)",       "docker ps 2>&1",       "CONTAINER")
        if not docker_img_ok:
            docker_img_ok  = await run_check("docker images (retry)",   "docker images 2>&1",   "REPOSITORY")
