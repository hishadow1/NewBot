...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y python-is-python3 2>/dev/null || \
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y pipx 2>/dev/null || true
RUN apt-get clean && rm -rf /var/lib/apt/lists/*
...
RUN cat > /etc/profile.d/00-darknodes-path.sh <<'PATHEOF'
#!/bin/sh
# Go
...
# npm globals (root uses /usr/local — always in PATH already)
# pip / python
...
RUN mkdir -p /etc/docker && cat > /etc/docker/daemon.json <<'DAEMONEOF'
{
